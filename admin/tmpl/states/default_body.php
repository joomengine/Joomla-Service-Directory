...
$edit = "index.php?option=com_servicedirectory&view=states&task=state.edit";
...
<?php foreach ($this->items as $i => $item): ?>
	<?php
		...
		$canDo = ServicedirectoryHelper::getActions('state',$item,'states');
	?>
	<tr class="row<?php echo $i % 2; ?>">
		<td class="order nowrap center hidden-phone">
		<?php if (!$this->isModal && $canDo->get('state.edit.state')): ?>
			<?php
				$iconClass = '';
				if (!$this->saveOrder)
				{
		...
		<?php if (!$this->isModal && $canDo->get('state.edit')): ?>
		...
		<td class="nowrap">
			<div class="name">
				<?php if (!$this->isModal && $canDo->get('state.edit')): ?>
					...
					<?php if ($item->checked_out): ?>
						<?php echo Html::_('jgrid.checkedout', $i, $userChkOut->name, $item->checked_out_time, 'states.', $canCheckin); ?>
					<?php endif; ?>
				<?php else: ?>
					<?php if (!$this->isModal): ?>
					...
					<?php else: ?>
						<?php
							...
							$attribs = 'data-content-select data-content-type="com_servicedirectory.state"'
								. ' data-id="' . $dataId . '"'
		...
		<?php if (!$this->isModal && $canDo->get('state.edit.state')) : ?>
				<?php if ($item->checked_out) : ?>
					<?php if ($canCheckin) : ?>
						<?php echo Html::_('jgrid.published', $item->published, $i, 'states.', true, 'cb'); ?>
					<?php else: ?>
						<?php echo Html::_('jgrid.published', $item->published, $i, 'states.', false, 'cb'); ?>
					<?php endif; ?>
				<?php else: ?>
					<?php echo Html::_('jgrid.published', $item->published, $i, 'states.', true, 'cb'); ?>
				<?php endif; ?>
