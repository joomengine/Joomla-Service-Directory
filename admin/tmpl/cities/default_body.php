...
$edit = "index.php?option=com_servicedirectory&view=cities&task=city.edit";
...
<?php foreach ($this->items as $i => $item): ?>
	...
	<tr class="row<?php echo $i % 2; ?>">
		...
		<?php if (!$this->isModal && $canDo->get('city.edit.state')): ?>
			<?php
				$iconClass = '';
				if (!$this->saveOrder)
				{
		...
		<?php if (!$this->isModal && $canDo->get('city.edit')): ?>
		...
		<td class="nowrap">
			<div class="name">
				<?php if (!$this->isModal && $canDo->get('city.edit')): ?>
					...
					<?php if ($item->checked_out): ?>
						<?php echo Html::_('jgrid.checkedout', $i, $userChkOut->name, $item->checked_out_time, 'cities.', $canCheckin); ?>
					<?php endif; ?>
				<?php else: ?>
					<?php if (!$this->isModal): ?>
					...
					<?php else: ?>
						<?php
							...
							$attribs = 'data-content-select data-content-type="com_servicedirectory.city"'
								. ' data-id="' . $dataId . '"'
		...
		<?php if (!$this->isModal && $canDo->get('city.edit.state')) : ?>
				<?php if ($item->checked_out) : ?>
					<?php if ($canCheckin) : ?>
						<?php echo Html::_('jgrid.published', $item->published, $i, 'cities.', true, 'cb'); ?>
					<?php else: ?>
						<?php echo Html::_('jgrid.published', $item->published, $i, 'cities.', false, 'cb'); ?>
					<?php endif; ?>
				<?php else: ?>
					<?php echo Html::_('jgrid.published', $item->published, $i, 'cities.', true, 'cb'); ?>
				<?php endif; ?>
