...
/**
 * Servicedirectory Language Admin Model
 *
 * @since  1.6
 */
class LanguageModel extends AdminModel
{
	...
	protected array $styles = [
		...
		'administrator/components/com_servicedirectory/assets/css/language.css'
 	];
	...
	protected array $scripts = [
		...
		'media/com_servicedirectory/js/language.js'
 	];
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_servicedirectory.language';
	 ...
	 * @throws  \Exception
	 */
	public function getTable($type = 'language', $prefix = 'Administrator', $config = [])
	{
	...
	public function getForm($data = [], $loadData = true, $options = ['control' => 'jform'])
	{
		...
		// Get the form.
		$form = $this->loadForm('com_servicedirectory.language', 'language', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('language.edit.state', 'com_servicedirectory.language.' . (int) $id))
			|| ($id == 0 && !$user->authorise('language.edit.state', 'com_servicedirectory')))
		{
	...
	protected function canDelete($record)
	{
		...
		return $this->getCurrentUser()->authorise('language.delete', 'com_servicedirectory.language.' . (int) $record->id);
	}
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('language.edit.state', 'com_servicedirectory.language.' . (int) $recordId);
		...
		return $user->authorise('language.edit.state', 'com_servicedirectory');
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__servicedirectory_language'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_servicedirectory.edit.language.data', []);
		...
		$this->preprocessData('com_servicedirectory.language', $data);
