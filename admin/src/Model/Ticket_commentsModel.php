...
/**
 * Ticket_comments List Model
 *
 * @since  1.6
 */
class Ticket_commentsModel extends ListModel
{
	/**
	...
	protected array $styles = [
		...
		'administrator/components/com_servicedirectory/assets/css/ticket_comments.css'
 	];
	...
	public function getItems()
	{
		...
		if (UtilitiesArrayHelper::check($items))
		{
			...
			foreach ($items as $nr => &$item)
			{
				...
				$access = ($user->authorise('ticket_comment.access', 'com_servicedirectory.ticket_comment.' . (int) $item->id) && $user->authorise('ticket_comment.access', 'com_servicedirectory'));
				if (!$access)
				{
					unset($items[$nr]);
					continue;
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__servicedirectory_ticket_comment', 'a'));

		// From the servicedirectory_ticket table.
		$query->select($db->quoteName(['g.subject','g.id'],['ticket_subject','ticket_id']));
		$query->join('LEFT', $db->quoteName('#__servicedirectory_ticket', 'g') . ' ON (' . $db->quoteName('a.ticket') . ' = ' . $db->quoteName('g.guid') . ')');
	...
	protected function checkInNow(): void
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__servicedirectory_ticket_comment'));
			...
			if ($db->getNumRows())
			{
				...
				// Check table.
				$query->update($db->quoteName('#__servicedirectory_ticket_comment'))->set($fields)->where($conditions); 
