...
class PlatformsModel extends ListModel
{
	...
	protected array $styles = [
		...
		'administrator/components/com_servicedirectory/assets/css/platforms.css'
 	];
	...
	public function getItems()
	{
		...
		if (UtilitiesArrayHelper::check($items))
		{
			...
			foreach ($items as $nr => &$item)
			{
				...
				$access = ($user->authorise('platform.access', 'com_servicedirectory.platform.' . (int) $item->id) && $user->authorise('platform.access', 'com_servicedirectory'));
				if (!$access)
				{
					unset($items[$nr]);
					continue;
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__servicedirectory_platform', 'a'));
	...
	protected function checkInNow(): void
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__servicedirectory_platform'));
			...
			if ($db->getNumRows())
			{
				...
				// Check table.
				$query->update($db->quoteName('#__servicedirectory_platform'))->set($fields)->where($conditions); 
