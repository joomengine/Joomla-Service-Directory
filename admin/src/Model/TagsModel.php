...
class TagsModel extends ListModel
{
	...
	protected array $styles = [
		...
		'administrator/components/com_servicedirectory/assets/css/tags.css'
 	];
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		if ($formSubmited)
		{
			$published = $input->post->get('published');
	...
	public function getItems()
	{
		...
		if (UtilitiesArrayHelper::check($items))
		{
			...
			foreach ($items as $nr => &$item)
			{
				...
				$access = ($user->authorise('tag.access', 'com_servicedirectory.tag.' . (int) $item->id) && $user->authorise('tag.access', 'com_servicedirectory'));
				if (!$access)
				{
					unset($items[$nr]);
					continue;
				...
				// convert parent_guid
				$item->parent_guid = JsonHelper::string($item->parent_guid, ', ', 'tag', 'guid', 'name');
			}
		}
	...
	public function selectionTranslation($value,$name)
	{
		...
		if ($name === 'published')
		{
			$publishedArray = array(
				1 => 'COM_SERVICEDIRECTORY_TAG_PUBLISHED',
				0 => 'COM_SERVICEDIRECTORY_TAG_UNPUBLISHED',
				2 => 'COM_SERVICEDIRECTORY_TAG_ARCHIVED',
				-2 => 'COM_SERVICEDIRECTORY_TAG_TRASHED'
			);
			// Now check if value is found in this array
			if (isset($publishedArray[$value]) && StringHelper::check($publishedArray[$value]))
			{
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__servicedirectory_tag', 'a'));

		// From the servicedirectory_tag table.
		$query->select($db->quoteName(['g.name','g.id'],['parent_guid_name','parent_guid_id']));
		$query->join('LEFT', $db->quoteName('#__servicedirectory_tag', 'g') . ' ON (' . $db->quoteName('a.parent_guid') . ' = ' . $db->quoteName('g.guid') . ')');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.name LIKE '.$search.' OR a.parent_guid LIKE '.$search.' OR g.name LIKE '.$search.' OR a.published LIKE '.$search.')');
			}
		}
		...
		{
			if (is_float($_published))
			{
				$query->where('a.published = ' . (float) $_published);
			}
			else
			{
				$query->where('a.published = ' . (int) $_published);
			}
		}
		elseif (StringHelper::check($_published))
		{
			$query->where('a.published = ' . $db->quote($db->escape($_published)));
		}
	...
	protected function checkInNow(): void
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__servicedirectory_tag'));
			...
			if ($db->getNumRows())
			{
				...
				// Check table.
				$query->update($db->quoteName('#__servicedirectory_tag'))->set($fields)->where($conditions); 
