...
/**
 * Servicedirectory Ticket Admin Model
 *
 * @since  1.6
 */
class TicketModel extends AdminModel
{
	...
	protected array $styles = [
		...
		'administrator/components/com_servicedirectory/assets/css/ticket.css'
 	];
	...
	protected array $scripts = [
		...
		'media/com_servicedirectory/js/ticket.js'
 	];
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_servicedirectory.ticket';
	...
	public function getForm($data = [], $loadData = true, $options = ['control' => 'jform'])
	{
		...
		// Get the form.
		$form = $this->loadForm('com_servicedirectory.ticket', 'ticket', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('ticket.edit.state', 'com_servicedirectory.ticket.' . (int) $id))
			|| ($id == 0 && !$user->authorise('ticket.edit.state', 'com_servicedirectory')))
		{
		...
		if ($id != 0 && (!$user->authorise('ticket.edit.created_by', 'com_servicedirectory.ticket.' . (int) $id))
			|| ($id == 0 && !$user->authorise('ticket.edit.created_by', 'com_servicedirectory')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('ticket.edit.created', 'com_servicedirectory.ticket.' . (int) $id))
			|| ($id == 0 && !$user->authorise('ticket.edit.created', 'com_servicedirectory')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('ticket.edit.access', 'com_servicedirectory.ticket.' . (int) $id))
			|| ($id == 0 && !$user->authorise('ticket.edit.access', 'com_servicedirectory')))
		{
			// Disable fields for display.
		...
		// Modify the form based on Edit Subject access controls.
		if ($id != 0 && (!$user->authorise('ticket.edit.subject', 'com_servicedirectory.ticket.' . (int) $id))
			|| ($id == 0 && !$user->authorise('ticket.edit.subject', 'com_servicedirectory')))
		{
			// Disable field on display.
			$form->setFieldAttribute('subject', 'disabled', 'true');
			// Make field readonly on display.
			$form->setFieldAttribute('subject', 'readonly', 'true');
			...
			if (!$form->getValue('subject'))
			{
				...
				// Disable field while saving.
				$form->setFieldAttribute('subject', 'required', 'false');
			}
		}
		// Modify the form based on Edit Company access controls.
		if ($id != 0 && (!$user->authorise('ticket.edit.company', 'com_servicedirectory.ticket.' . (int) $id))
			|| ($id == 0 && !$user->authorise('ticket.edit.company', 'com_servicedirectory')))
		{
			// Disable field on display.
		...
		if ($id != 0 && (!$user->authorise('ticket.edit.comment', 'com_servicedirectory.ticket.' . (int) $id))
			|| ($id == 0 && !$user->authorise('ticket.edit.comment', 'com_servicedirectory')))
		{
			// Disable field on display.
	...
	protected function canDelete($record)
	{
		...
		return $this->getCurrentUser()->authorise('ticket.delete', 'com_servicedirectory.ticket.' . (int) $record->id);
	}
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('ticket.edit.state', 'com_servicedirectory.ticket.' . (int) $recordId);
		...
		return $user->authorise('ticket.edit.state', 'com_servicedirectory');
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__servicedirectory_ticket'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_servicedirectory.edit.ticket.data', []);
		...
		$this->preprocessData('com_servicedirectory.ticket', $data);
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		elseif (isset($values['published']) && !$this->canDo->get('ticket.edit.state'))
		{
				$values['published'] = 0;
		}
		...
		while (!empty($pks))
		{
			...
			if (!$this->user->authorise('ticket.edit', $contexts[$pk]))
			{
				// Not fatal error
			...
			// Only for strings
			if (UtilitiesStringHelper::check($this->table->subject) && !is_numeric($this->table->subject))
			{
				$this->table->subject = $this->generateUnique('subject',$this->table->subject);
			}
	...
	protected function batchMove($values, $pks, $contexts)
	{
		...
		if (isset($values['published']) && !$this->canDo->get('ticket.edit.state'))
		{
	...
	public function save($data)
	{
		...
		if (empty($data['guid']) && $data['id'] > 0)
		{
			...
			$data['guid'] = (string) GetHelper::var('ticket', $data['id'], 'id', 'guid');
		}

		// Set the GUID if empty or not valid
		while (!GuidHelper::valid($data['guid'], "ticket", $data['id']))
		{
			// must always be set
