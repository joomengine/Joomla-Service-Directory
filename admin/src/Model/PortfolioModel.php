...
/***[/INSERTED$$$$]***/
	...
	protected function allowEdit($data = [], $key = 'id')
	{
		...
		return $user->authorise('portfolio.edit', 'com_servicedirectory.portfolio.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or $user->authorise('portfolio.edit',  'com_servicedirectory');
	}
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		elseif (isset($values['published']) && !$this->canDo->get('portfolio.edit.state'))
		{
				$values['published'] = 0;
		}
		...
		while (!empty($pks))
		{
			...
			if (!$this->user->authorise('portfolio.edit', $contexts[$pk]))
			{
				// Not fatal error
			...
			// Only for strings
			if (UtilitiesStringHelper::check($this->table->project_title) && !is_numeric($this->table->project_title))
			{
				$this->table->project_title = $this->generateUnique('project_title',$this->table->project_title);
			}
	...
	protected function batchMove($values, $pks, $contexts)
	{
		...
		if (isset($values['published']) && !$this->canDo->get('portfolio.edit.state'))
		{
...
/***[JCBGUI.admin_view.php_before_save.332.$$$$]***/
