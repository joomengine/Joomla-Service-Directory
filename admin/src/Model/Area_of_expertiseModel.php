...
/**
 * Servicedirectory Area_of_expertise Admin Model
 *
 * @since  1.6
 */
class Area_of_expertiseModel extends AdminModel
{
	...
	protected array $styles = [
		...
		'administrator/components/com_servicedirectory/assets/css/area_of_expertise.css'
 	];
	...
	protected array $scripts = [
		...
		'media/com_servicedirectory/js/area_of_expertise.js'
 	];
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_servicedirectory.area_of_expertise';
	 ...
	 * @throws  \Exception
	 */
	public function getTable($type = 'area_of_expertise', $prefix = 'Administrator', $config = [])
	{
	...
	public function getForm($data = [], $loadData = true, $options = ['control' => 'jform'])
	{
		...
		// Get the form.
		$form = $this->loadForm('com_servicedirectory.area_of_expertise', 'area_of_expertise', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('area_of_expertise.edit.state', 'com_servicedirectory.area_of_expertise.' . (int) $id))
			|| ($id == 0 && !$user->authorise('area_of_expertise.edit.state', 'com_servicedirectory')))
		{
		...
		if ($id != 0 && (!$user->authorise('area_of_expertise.edit.created_by', 'com_servicedirectory.area_of_expertise.' . (int) $id))
			|| ($id == 0 && !$user->authorise('area_of_expertise.edit.created_by', 'com_servicedirectory')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('area_of_expertise.edit.created', 'com_servicedirectory.area_of_expertise.' . (int) $id))
			|| ($id == 0 && !$user->authorise('area_of_expertise.edit.created', 'com_servicedirectory')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('area_of_expertise.edit.name', 'com_servicedirectory.area_of_expertise.' . (int) $id))
			|| ($id == 0 && !$user->authorise('area_of_expertise.edit.name', 'com_servicedirectory')))
		{
		...
		if ($id != 0 && (!$user->authorise('area_of_expertise.edit.description', 'com_servicedirectory.area_of_expertise.' . (int) $id))
			|| ($id == 0 && !$user->authorise('area_of_expertise.edit.description', 'com_servicedirectory')))
		{
			// Disable field on display.
		...
		if ($id != 0 && (!$user->authorise('area_of_expertise.edit.file_type', 'com_servicedirectory.area_of_expertise.' . (int) $id))
			|| ($id == 0 && !$user->authorise('area_of_expertise.edit.file_type', 'com_servicedirectory')))
		{
			// Disable field on display.
		...
		if ($id != 0 && (!$user->authorise('area_of_expertise.edit.alias', 'com_servicedirectory.area_of_expertise.' . (int) $id))
			|| ($id == 0 && !$user->authorise('area_of_expertise.edit.alias', 'com_servicedirectory')))
		{
			// Disable field on display.
	...
	protected function canDelete($record)
	{
		...
		return $this->getCurrentUser()->authorise('area_of_expertise.delete', 'com_servicedirectory.area_of_expertise.' . (int) $record->id);
	}
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('area_of_expertise.edit.state', 'com_servicedirectory.area_of_expertise.' . (int) $recordId);
		...
		return $user->authorise('area_of_expertise.edit.state', 'com_servicedirectory');
	}
	...
	protected function allowEdit(array $data = [], string $key = 'id'): bool
	{
		...
		// Access check.
		$access = ($user->authorise('area_of_expertise.access', 'com_servicedirectory.area_of_expertise.' . (int) $recordId) && $user->authorise('area_of_expertise.access', 'com_servicedirectory'));
		if (!$access)
		{
			return false;
		}
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('area_of_expertise.edit', 'com_servicedirectory.area_of_expertise.' . (int) $recordId);
			if (!$permission)
			{
				if ($user->authorise('area_of_expertise.edit.own', 'com_servicedirectory.area_of_expertise.' . $recordId))
				{
					...
					if ($ownerId == $user->id)
					{
						if ($user->authorise('area_of_expertise.edit.own', 'com_servicedirectory'))
						{
							return true;
						}
					}
		...
		return $user->authorise('area_of_expertise.edit', $this->option);
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__servicedirectory_area_of_expertise'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_servicedirectory.edit.area_of_expertise.data', []);
		...
		$this->preprocessData('com_servicedirectory.area_of_expertise', $data);
	...
	public function delete(&$pks)
	{
		...
		if (!empty($_tables_array) && UtilitiesArrayHelper::check($pks))
		{
			...
			{
				// get the area_of_expertise field key's
				$_guids = DataFactory::_('Load')->values(
					['a.guid' => $_field_key], // select
					['a' => 'area_of_expertise'], // tables
					['a.id' =>
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		elseif (isset($values['published']) && !$this->canDo->get('area_of_expertise.edit.state'))
		{
				$values['published'] = 0;
		}
		...
		while (!empty($pks))
		{
			...
			if (!$this->user->authorise('area_of_expertise.edit', $contexts[$pk]))
			{
				// Not fatal error
	...
	protected function batchMove($values, $pks, $contexts)
	{
		...
		if (isset($values['published']) && !$this->canDo->get('area_of_expertise.edit.state'))
		{
	...
	public function save($data)
	{
		...
		if (empty($data['guid']) && $data['id'] > 0)
		{
			...
			$data['guid'] = (string) GetHelper::var('area_of_expertise', $data['id'], 'id', 'guid');
		}

		// Set the GUID if empty or not valid
		while (!GuidHelper::valid($data['guid'], "area_of_expertise", $data['id']))
		{
			// must always be set
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null || empty($data['alias']))
			{
				...
				{
					$msg = Text::_('COM_SERVICEDIRECTORY_AREA_OF_EXPERTISE_SAVE_WARNING');
				}
