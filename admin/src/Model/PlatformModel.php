...
/**
 * Servicedirectory Platform Admin Model
 *
 * @since  1.6
 */
class PlatformModel extends AdminModel
{
	...
	protected array $styles = [
		...
		'administrator/components/com_servicedirectory/assets/css/platform.css'
 	];
	...
	protected array $scripts = [
		...
		'media/com_servicedirectory/js/platform.js'
 	];
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_servicedirectory.platform';
	 ...
	 * @throws  \Exception
	 */
	public function getTable($type = 'platform', $prefix = 'Administrator', $config = [])
	{
	...
	public function getForm($data = [], $loadData = true, $options = ['control' => 'jform'])
	{
		...
		// Get the form.
		$form = $this->loadForm('com_servicedirectory.platform', 'platform', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('platform.edit.state', 'com_servicedirectory.platform.' . (int) $id))
			|| ($id == 0 && !$user->authorise('platform.edit.state', 'com_servicedirectory')))
		{
		...
		if ($id != 0 && (!$user->authorise('platform.edit.created_by', 'com_servicedirectory.platform.' . (int) $id))
			|| ($id == 0 && !$user->authorise('platform.edit.created_by', 'com_servicedirectory')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('platform.edit.created', 'com_servicedirectory.platform.' . (int) $id))
			|| ($id == 0 && !$user->authorise('platform.edit.created', 'com_servicedirectory')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('platform.edit.access', 'com_servicedirectory.platform.' . (int) $id))
			|| ($id == 0 && !$user->authorise('platform.edit.access', 'com_servicedirectory')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('platform.edit.name', 'com_servicedirectory.platform.' . (int) $id))
			|| ($id == 0 && !$user->authorise('platform.edit.name', 'com_servicedirectory')))
		{
		...
		if ($id != 0 && (!$user->authorise('platform.edit.website', 'com_servicedirectory.platform.' . (int) $id))
			|| ($id == 0 && !$user->authorise('platform.edit.website', 'com_servicedirectory')))
		{
			// Disable field on display.
		...
		if ($id != 0 && (!$user->authorise('platform.edit.icon', 'com_servicedirectory.platform.' . (int) $id))
			|| ($id == 0 && !$user->authorise('platform.edit.icon', 'com_servicedirectory')))
		{
			// Disable field on display.
		...
		if ($id != 0 && (!$user->authorise('platform.edit.guid', 'com_servicedirectory.platform.' . (int) $id))
			|| ($id == 0 && !$user->authorise('platform.edit.guid', 'com_servicedirectory')))
		{
			// Disable field on display.
		...
		if ($id != 0 && (!$user->authorise('platform.edit.file_type', 'com_servicedirectory.platform.' . (int) $id))
			|| ($id == 0 && !$user->authorise('platform.edit.file_type', 'com_servicedirectory')))
		{
			// Disable field on display.
	...
	protected function canDelete($record)
	{
		...
		return $this->getCurrentUser()->authorise('platform.delete', 'com_servicedirectory.platform.' . (int) $record->id);
	}
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('platform.edit.state', 'com_servicedirectory.platform.' . (int) $recordId);
		...
		return $user->authorise('platform.edit.state', 'com_servicedirectory');
	}
	...
	protected function allowEdit(array $data = [], string $key = 'id'): bool
	{
		...
		// Access check.
		$access = ($user->authorise('platform.access', 'com_servicedirectory.platform.' . (int) $recordId) && $user->authorise('platform.access', 'com_servicedirectory'));
		if (!$access)
		{
			return false;
		}
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('platform.edit', 'com_servicedirectory.platform.' . (int) $recordId);
			if (!$permission)
			{
				if ($user->authorise('platform.edit.own', 'com_servicedirectory.platform.' . $recordId))
				{
					...
					if ($ownerId == $user->id)
					{
						if ($user->authorise('platform.edit.own', 'com_servicedirectory'))
						{
							return true;
						}
					}
		...
		return $user->authorise('platform.edit', $this->option);
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__servicedirectory_platform'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_servicedirectory.edit.platform.data', []);
		...
		$this->preprocessData('com_servicedirectory.platform', $data);
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		elseif (isset($values['published']) && !$this->canDo->get('platform.edit.state'))
		{
				$values['published'] = 0;
		}
		...
		while (!empty($pks))
		{
			...
			if (!$this->user->authorise('platform.edit', $contexts[$pk]))
			{
				// Not fatal error
	...
	protected function batchMove($values, $pks, $contexts)
	{
		...
		if (isset($values['published']) && !$this->canDo->get('platform.edit.state'))
		{
	...
	public function save($data)
	{
		...
		if (empty($data['guid']) && $data['id'] > 0)
		{
			...
			$data['guid'] = (string) GetHelper::var('platform', $data['id'], 'id', 'guid');
		}

		// Set the GUID if empty or not valid
		while (!GuidHelper::valid($data['guid'], "platform", $data['id']))
		{
			// must always be set
