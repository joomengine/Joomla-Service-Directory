...
/**
 * Company_tags List Model
 *
 * @since  1.6
 */
class Company_tagsModel extends ListModel
{
	/**
	...
	protected array $styles = [
		...
		'administrator/components/com_servicedirectory/assets/css/company_tags.css'
 	];
	...
	public function getItems()
	{
		...
		if (UtilitiesArrayHelper::check($items))
		{
			...
			foreach ($items as $nr => &$item)
			{
				...
				$access = ($user->authorise('company_tag.access', 'com_servicedirectory.company_tag.' . (int) $item->id) && $user->authorise('company_tag.access', 'com_servicedirectory'));
				if (!$access)
				{
					unset($items[$nr]);
					continue;
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__servicedirectory_company_tag', 'a'));

		// From the servicedirectory_tag table.
		$query->select($db->quoteName(['g.name','g.id'],['tag_name','tag_id']));
		$query->join('LEFT', $db->quoteName('#__servicedirectory_tag', 'g') . ' ON (' . $db->quoteName('a.tag') . ' = ' . $db->quoteName('g.guid') . ')');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.tag LIKE '.$search.' OR g.name LIKE '.$search.' OR a.company LIKE '.$search.' OR h.name LIKE '.$search.')');
			}
		}
	...
	public function getPrivacyExport($pks, $user = null)
	{
		...
		if (($pks_size = UtilitiesArrayHelper::check($pks)) !== false || 'bulk' === $pks)
		{
			...
			// From the servicedirectory_company_tag table
			$query->from($db->quoteName('#__servicedirectory_company_tag', 'a'));
			...
			if ($export_text_only)
			{
				...
				// From the servicedirectory_tag table.
				$query->select($db->quoteName(['g.name','g.id'],['tag','tag_id']));
				$query->join('LEFT', $db->quoteName('#__servicedirectory_tag', 'g') . ' ON (' . $db->quoteName('a.tag') . ' = ' . $db->quoteName('g.guid') . ')');
	...
	protected function checkInNow(): void
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__servicedirectory_company_tag'));
			...
			if ($db->getNumRows())
			{
				...
				// Check table.
				$query->update($db->quoteName('#__servicedirectory_company_tag'))->set($fields)->where($conditions); 
