...
/**
 * Areas_of_expertise List Model
 *
 * @since  1.6
 */
class Areas_of_expertiseModel extends ListModel
{
	/**
	...
	protected array $styles = [
		...
		'administrator/components/com_servicedirectory/assets/css/areas_of_expertise.css'
 	];
	...
	public function getItems()
	{
		...
		if (UtilitiesArrayHelper::check($items))
		{
			...
			foreach ($items as $nr => &$item)
			{
				...
				$access = ($user->authorise('area_of_expertise.access', 'com_servicedirectory.area_of_expertise.' . (int) $item->id) && $user->authorise('area_of_expertise.access', 'com_servicedirectory'));
				if (!$access)
				{
					unset($items[$nr]);
					continue;
	...
	protected function checkInNow(): void
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__servicedirectory_area_of_expertise'));
			...
			if ($db->getNumRows())
			{
				...
				// Check table.
				$query->update($db->quoteName('#__servicedirectory_area_of_expertise'))->set($fields)->where($conditions); 
