...
/**
 * Review_company_updates List Model
 *
 * @since  1.6
 */
class Review_company_updatesModel extends ListModel
{
	/**
	...
	protected array $styles = [
		...
		'administrator/components/com_servicedirectory/assets/css/review_company_updates.css'
 	];
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$review_status = $this->getUserStateFromRequest($this->context . '.filter.review_status', 'filter_review_status');
		if ($formSubmited)
		{
			$review_status = $input->post->get('review_status');
			$this->setState('filter.review_status', $review_status);
		}
	...
	public function getItems()
	{
		...
		if (UtilitiesArrayHelper::check($items))
		{
			...
			{
				// convert review_status
				$item->review_status = $this->selectionTranslation($item->review_status, 'review_status');
			}
		}
	...
	{
		// Array of review_status language strings
		if ($name === 'review_status')
		{
			$review_statusArray = array(
				0 => 'COM_SERVICEDIRECTORY_REVIEW_COMPANY_UPDATE_PENDING',
				1 => 'COM_SERVICEDIRECTORY_REVIEW_COMPANY_UPDATE_APPROVED',
				2 => 'COM_SERVICEDIRECTORY_REVIEW_COMPANY_UPDATE_ON_HOLD',
				3 => 'COM_SERVICEDIRECTORY_REVIEW_COMPANY_UPDATE_DENIED'
			);
			// Now check if value is found in this array
			if (isset($review_statusArray[$value]) && StringHelper::check($review_statusArray[$value]))
			{
				return $review_statusArray[$value];
			}
		}
		return $value;
	}
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__servicedirectory_review_company_update', 'a'));
		...
		$_review_status = $this->getState('filter.review_status');
		if (is_numeric($_review_status))
		{
			if (is_float($_review_status))
			{
				$query->where('a.review_status = ' . (float) $_review_status);
			}
			else
			{
				$query->where('a.review_status = ' . (int) $_review_status);
			}
		}
		elseif (StringHelper::check($_review_status))
		{
			$query->where('a.review_status = ' . $db->quote($db->escape($_review_status)));
		}
	...
	public function getPrivacyExport($pks, $user = null)
	{
		...
		if (($pks_size = UtilitiesArrayHelper::check($pks)) !== false || 'bulk' === $pks)
		{
			...
			// From the servicedirectory_review_company_update table
			$query->from($db->quoteName('#__servicedirectory_review_company_update', 'a'));
			...
			if ($export_text_only)
			{
				...
				// From the servicedirectory_language table.
				$query->select($db->quoteName(['i.name','i.id'],['languages','languages_id']));
				$query->join('LEFT', $db->quoteName('#__servicedirectory_language', 'i') . ' ON (' . $db->quoteName('a.languages') . ' = ' . $db->quoteName('i.langtag') . ')');

				// From the servicedirectory_tag table.
				$query->select($db->quoteName(['j.name','j.id'],['tags','tags_id']));
				$query->join('LEFT', $db->quoteName('#__servicedirectory_tag', 'j') . ' ON (' . $db->quoteName('a.tags') . ' = ' . $db->quoteName('j.guid') . ')');

				// From the servicedirectory_area_of_expertise table.
				$query->select($db->quoteName(['k.name','k.id'],['areas_of_expertise','areas_of_expertise_id']));
				$query->join('LEFT', $db->quoteName('#__servicedirectory_area_of_expertise', 'k') . ' ON (' . $db->quoteName('a.areas_of_expertise') . ' = ' . $db->quoteName('k.guid') . ')');
			}
			// Implement View Level Access
			if (!$user->authorise('core.options', 'com_servicedirectory'))
			{
			...
			if ($db->getNumRows())
			{
				...
				if (UtilitiesArrayHelper::check($items))
				{
					...
					{
						// convert addresses
						$item->addresses = JsonHelper::string($item->addresses, ', ', 'addresses');
						// convert languages
						$item->languages = JsonHelper::string($item->languages, ', ', 'language', 'langtag', 'name');
						// convert tags
						$item->tags = JsonHelper::string($item->tags, ', ', 'tag', 'guid', 'name');
						// convert areas_of_expertise
						$item->areas_of_expertise = JsonHelper::string($item->areas_of_expertise, ', ', 'area_of_expertise', 'guid', 'name');
						// convert portfolios
						$item->portfolios = JsonHelper::string($item->portfolios, ', ', 'portfolios');
						// convert social_handles
						$item->social_handles = JsonHelper::string($item->social_handles, ', ', 'social_handles');
					}
				}
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':' . $this->getState('filter.review_status');
	...
	protected function checkInNow(): void
	{
		...
		if ($time)
		{
			...
			if ($db->getNumRows())
			{
				...
				// Check table.
				$query->update($db->quoteName('#__servicedirectory_review_company_update'))->set($fields)->where($conditions); 
