...
class CategoriesModel extends ListModel
{
	...
	protected array $styles = [
		...
		'administrator/components/com_servicedirectory/assets/css/categories.css'
 	];
	...
	public function getItems()
	{
		...
		if (UtilitiesArrayHelper::check($items))
		{
			...
			foreach ($items as $nr => &$item)
			{
				...
				$access = ($user->authorise('category.access', 'com_servicedirectory.category.' . (int) $item->id) && $user->authorise('category.access', 'com_servicedirectory'));
				if (!$access)
				{
					unset($items[$nr]);
					continue;
	...
	public function selectionTranslation($value,$name)
	{
		...
		if ($name === 'published')
		{
			$publishedArray = array(
				1 => 'COM_SERVICEDIRECTORY_CATEGORY_PUBLISHED',
				0 => 'COM_SERVICEDIRECTORY_CATEGORY_UNPUBLISHED',
				2 => 'COM_SERVICEDIRECTORY_CATEGORY_ARCHIVED',
				-2 => 'COM_SERVICEDIRECTORY_CATEGORY_TRASHED'
			);
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__servicedirectory_category', 'a'));

		// From the servicedirectory_category table.
		$query->select($db->quoteName(['g.name','g.id'],['parent_guid_name','parent_guid_id']));
		$query->join('LEFT', $db->quoteName('#__servicedirectory_category', 'g') . ' ON (' . $db->quoteName('a.parent_guid') . ' = ' . $db->quoteName('g.guid') . ')');
	...
	protected function checkInNow(): void
	{
		...
		if ($time)
		{
			...
			if ($db->getNumRows())
			{
				...
				// Check table.
				$query->update($db->quoteName('#__servicedirectory_category'))->set($fields)->where($conditions); 
