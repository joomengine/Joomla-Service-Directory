...
/**
 * Servicedirectory Category Admin Model
 *
 * @since  1.6
 */
class CategoryModel extends AdminModel
{
	...
	protected array $styles = [
		...
		'administrator/components/com_servicedirectory/assets/css/category.css'
 	];
	...
	protected array $scripts = [
		...
		'media/com_servicedirectory/js/category.js'
 	];
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_servicedirectory.category';
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->parent_guid))
			{
				// Convert the parent_guid field to an array.
				$parent_guid = new Registry;
				$parent_guid->loadString($item->parent_guid);
				$item->parent_guid = $parent_guid->toArray();
			}
		}
	...
	public function getForm($data = [], $loadData = true, $options = ['control' => 'jform'])
	{
		...
		// Get the form.
		$form = $this->loadForm('com_servicedirectory.category', 'category', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('category.edit.state', 'com_servicedirectory.category.' . (int) $id))
			|| ($id == 0 && !$user->authorise('category.edit.state', 'com_servicedirectory')))
		{
		...
		if ($id != 0 && (!$user->authorise('category.edit.created_by', 'com_servicedirectory.category.' . (int) $id))
			|| ($id == 0 && !$user->authorise('category.edit.created_by', 'com_servicedirectory')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('category.edit.created', 'com_servicedirectory.category.' . (int) $id))
			|| ($id == 0 && !$user->authorise('category.edit.created', 'com_servicedirectory')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('category.edit.name', 'com_servicedirectory.category.' . (int) $id))
			|| ($id == 0 && !$user->authorise('category.edit.name', 'com_servicedirectory')))
		{
		...
		if ($id != 0 && (!$user->authorise('category.edit.description', 'com_servicedirectory.category.' . (int) $id))
			|| ($id == 0 && !$user->authorise('category.edit.description', 'com_servicedirectory')))
		{
			// Disable field on display.
		...
		if ($id != 0 && (!$user->authorise('category.edit.guid', 'com_servicedirectory.category.' . (int) $id))
			|| ($id == 0 && !$user->authorise('category.edit.guid', 'com_servicedirectory')))
		{
			// Disable field on display.
		...
		if ($id != 0 && (!$user->authorise('category.edit.file_type', 'com_servicedirectory.category.' . (int) $id))
			|| ($id == 0 && !$user->authorise('category.edit.file_type', 'com_servicedirectory')))
		{
			// Disable field on display.
	...
	protected function canDelete($record)
	{
		...
		return $this->getCurrentUser()->authorise('category.delete', 'com_servicedirectory.category.' . (int) $record->id);
	}
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('category.edit.state', 'com_servicedirectory.category.' . (int) $recordId);
		...
		return $user->authorise('category.edit.state', 'com_servicedirectory');
	}
	...
	protected function allowEdit(array $data = [], string $key = 'id'): bool
	{
		...
		// Access check.
		$access = ($user->authorise('category.access', 'com_servicedirectory.category.' . (int) $recordId) && $user->authorise('category.access', 'com_servicedirectory'));
		if (!$access)
		{
			return false;
		}
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('category.edit', 'com_servicedirectory.category.' . (int) $recordId);
			if (!$permission)
			{
				if ($user->authorise('category.edit.own', 'com_servicedirectory.category.' . $recordId))
				{
					...
					if ($ownerId == $user->id)
					{
						if ($user->authorise('category.edit.own', 'com_servicedirectory'))
						{
							return true;
						}
					}
		...
		return $user->authorise('category.edit', $this->option);
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__servicedirectory_category'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_servicedirectory.edit.category.data', []);
		...
		$this->preprocessData('com_servicedirectory.category', $data);
	...
	public function batch($commands, $pks, $contexts)
	{
		...
		$this->canDo = ServicedirectoryHelper::getActions('category');
		$this->batchSet = true;
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		if (empty($this->batchSet))
		{
			...
			$this->table 		= $this->getTable();
			$this->tableClassName	= get_class($this->table);
			$this->canDo		= Actions::get('category');
		}

		if (!$this->canDo->get('category.create') && !$this->canDo->get('category.batch'))
		{
			return false;
		}
		...
		elseif (isset($values['published']) && !$this->canDo->get('category.edit.state'))
		{
				$values['published'] = 0;
		}
		...
		while (!empty($pks))
		{
			...
			if (!$this->user->authorise('category.edit', $contexts[$pk]))
			{
				// Not fatal error
	...
	protected function batchMove($values, $pks, $contexts)
	{
		...
		if (!$this->canDo->get('category.edit') && !$this->canDo->get('category.batch'))
		{
		...
		if (isset($values['published']) && !$this->canDo->get('category.edit.state'))
		{
	...
	public function save($data)
	{
		...
		if (empty($data['guid']) && $data['id'] > 0)
		{
			...
			$data['guid'] = (string) GetHelper::var('category', $data['id'], 'id', 'guid');
		}

		// Set the GUID if empty or not valid
		while (!GuidHelper::valid($data['guid'], "category", $data['id']))
		{
			// must always be set
		...
		{
			$parent_guid = new Registry;
			$parent_guid->loadArray($data['parent_guid']);
			$data['parent_guid'] = (string) $parent_guid;
		}
		elseif (!isset($data['parent_guid']))
		{
			// Set the empty parent_guid to data
			$data['parent_guid'] = '';
		}
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null || empty($data['alias']))
			{
				...
				{
					$msg = Text::_('COM_SERVICEDIRECTORY_CATEGORY_SAVE_WARNING');
				}
