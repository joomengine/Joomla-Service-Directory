 ...
 * @since  1.6
 */
class CitiesModel extends ListModel
{
	/**
	...
	protected array $styles = [
		...
		'administrator/components/com_servicedirectory/assets/css/cities.css'
 	];
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$latitude = $this->getUserStateFromRequest($this->context . '.filter.latitude', 'filter_latitude');
		if ($formSubmited)
		{
			$latitude = $input->post->get('latitude');
			$this->setState('filter.latitude', $latitude);
		}

		$longitude = $this->getUserStateFromRequest($this->context . '.filter.longitude', 'filter_longitude');
		if ($formSubmited)
		{
			$longitude = $input->post->get('longitude');
			$this->setState('filter.longitude', $longitude);
		}
	...
	public function getItems()
	{
		...
		if (UtilitiesArrayHelper::check($items))
		{
			...
			foreach ($items as $nr => &$item)
			{
				...
				$access = ($user->authorise('city.access', 'com_servicedirectory.city.' . (int) $item->id) && $user->authorise('city.access', 'com_servicedirectory'));
				if (!$access)
				{
					unset($items[$nr]);
					continue;
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__servicedirectory_city', 'a'));

		// From the servicedirectory_state table.
		$query->select($db->quoteName(['g.name','g.id'],['state_name','state_id']));
		$query->join('LEFT', $db->quoteName('#__servicedirectory_state', 'g') . ' ON (' . $db->quoteName('a.state') . ' = ' . $db->quoteName('g.guid') . ')');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.name LIKE '.$search.' OR a.state LIKE '.$search.' OR g.name LIKE '.$search.' OR a.latitude LIKE '.$search.' OR a.longitude LIKE '.$search.' OR a.wikidataid LIKE '.$search.')');
			}
		}
	...
	protected function checkInNow(): void
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__servicedirectory_city'));
			...
			if ($db->getNumRows())
			{
				...
				// Check table.
				$query->update($db->quoteName('#__servicedirectory_city'))->set($fields)->where($conditions); 
