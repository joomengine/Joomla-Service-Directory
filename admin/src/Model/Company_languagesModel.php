...
/**
 * Company_languages List Model
 *
 * @since  1.6
 */
class Company_languagesModel extends ListModel
{
	/**
	...
	protected array $styles = [
		...
		'administrator/components/com_servicedirectory/assets/css/company_languages.css'
 	];
	...
	public function getItems()
	{
		...
		if (UtilitiesArrayHelper::check($items))
		{
			...
			foreach ($items as $nr => &$item)
			{
				...
				$access = ($user->authorise('company_language.access', 'com_servicedirectory.company_language.' . (int) $item->id) && $user->authorise('company_language.access', 'com_servicedirectory'));
				if (!$access)
				{
					unset($items[$nr]);
					continue;
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__servicedirectory_company_language', 'a'));

		// From the servicedirectory_language table.
		$query->select($db->quoteName(['g.name','g.id'],['language_name','language_id']));
		$query->join('LEFT', $db->quoteName('#__servicedirectory_language', 'g') . ' ON (' . $db->quoteName('a.language') . ' = ' . $db->quoteName('g.langtag') . ')');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.language LIKE '.$search.' OR g.name LIKE '.$search.' OR a.company LIKE '.$search.' OR h.name LIKE '.$search.')');
			}
		}
	...
	public function getPrivacyExport($pks, $user = null)
	{
		...
		if (($pks_size = UtilitiesArrayHelper::check($pks)) !== false || 'bulk' === $pks)
		{
			...
			// From the servicedirectory_company_language table
			$query->from($db->quoteName('#__servicedirectory_company_language', 'a'));
			...
			if ($export_text_only)
			{
				...
				// From the servicedirectory_language table.
				$query->select($db->quoteName(['g.name','g.id'],['language','language_id']));
				$query->join('LEFT', $db->quoteName('#__servicedirectory_language', 'g') . ' ON (' . $db->quoteName('a.language') . ' = ' . $db->quoteName('g.langtag') . ')');
	...
	protected function checkInNow(): void
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__servicedirectory_company_language'));
			...
			if ($db->getNumRows())
			{
				...
				// Check table.
				$query->update($db->quoteName('#__servicedirectory_company_language'))->set($fields)->where($conditions); 
