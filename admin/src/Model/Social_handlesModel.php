...
/**
 * Social_handles List Model
 *
 * @since  1.6
 */
class Social_handlesModel extends ListModel
{
	/**
	...
	protected array $styles = [
		...
		'administrator/components/com_servicedirectory/assets/css/social_handles.css'
 	];
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		if ($formSubmited)
		{
			$platform = $input->post->get('platform');
	...
	public function getItems()
	{
		...
		if (UtilitiesArrayHelper::check($items))
		{
			...
			foreach ($items as $nr => &$item)
			{
				...
				$access = ($user->authorise('social_handle.access', 'com_servicedirectory.social_handle.' . (int) $item->id) && $user->authorise('social_handle.access', 'com_servicedirectory'));
				if (!$access)
				{
					unset($items[$nr]);
					continue;
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__servicedirectory_social_handle', 'a'));

		// From the servicedirectory_platform table.
		$query->select($db->quoteName(['g.name','g.id'],['platform_name','platform_id']));
		$query->join('LEFT', $db->quoteName('#__servicedirectory_platform', 'g') . ' ON (' . $db->quoteName('a.platform') . ' = ' . $db->quoteName('g.guid') . ')');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.platform LIKE '.$search.' OR g.name LIKE '.$search.' OR a.company LIKE '.$search.' OR h.name LIKE '.$search.')');
			}
		}
	...
	public function getPrivacyExport($pks, $user = null)
	{
		...
		if (($pks_size = UtilitiesArrayHelper::check($pks)) !== false || 'bulk' === $pks)
		{
			...
			// From the servicedirectory_social_handle table
			$query->from($db->quoteName('#__servicedirectory_social_handle', 'a'));
			...
			if ($export_text_only)
			{
				...
				// From the servicedirectory_platform table.
				$query->select($db->quoteName(['g.name','g.id'],['platform','platform_id']));
				$query->join('LEFT', $db->quoteName('#__servicedirectory_platform', 'g') . ' ON (' . $db->quoteName('a.platform') . ' = ' . $db->quoteName('g.guid') . ')');
	...
	protected function checkInNow(): void
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__servicedirectory_social_handle'));
			...
			if ($db->getNumRows())
			{
				...
				// Check table.
				$query->update($db->quoteName('#__servicedirectory_social_handle'))->set($fields)->where($conditions); 
