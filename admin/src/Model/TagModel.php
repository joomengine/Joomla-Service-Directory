...
/**
 * Servicedirectory Tag Admin Model
 *
 * @since  1.6
 */
class TagModel extends AdminModel
{
	...
	protected array $styles = [
		...
		'administrator/components/com_servicedirectory/assets/css/tag.css'
 	];
	...
	protected array $scripts = [
		...
		'media/com_servicedirectory/js/tag.js'
 	];
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_servicedirectory.tag';
	...
	public function getForm($data = [], $loadData = true, $options = ['control' => 'jform'])
	{
		...
		// Get the form.
		$form = $this->loadForm('com_servicedirectory.tag', 'tag', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('tag.edit.state', 'com_servicedirectory.tag.' . (int) $id))
			|| ($id == 0 && !$user->authorise('tag.edit.state', 'com_servicedirectory')))
		{
		...
		if ($id != 0 && (!$user->authorise('tag.edit.created_by', 'com_servicedirectory.tag.' . (int) $id))
			|| ($id == 0 && !$user->authorise('tag.edit.created_by', 'com_servicedirectory')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('tag.edit.created', 'com_servicedirectory.tag.' . (int) $id))
			|| ($id == 0 && !$user->authorise('tag.edit.created', 'com_servicedirectory')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('tag.edit.name', 'com_servicedirectory.tag.' . (int) $id))
			|| ($id == 0 && !$user->authorise('tag.edit.name', 'com_servicedirectory')))
		{
		...
		if ($id != 0 && (!$user->authorise('tag.edit.description', 'com_servicedirectory.tag.' . (int) $id))
			|| ($id == 0 && !$user->authorise('tag.edit.description', 'com_servicedirectory')))
		{
			// Disable field on display.
		...
		if ($id != 0 && (!$user->authorise('tag.edit.guid', 'com_servicedirectory.tag.' . (int) $id))
			|| ($id == 0 && !$user->authorise('tag.edit.guid', 'com_servicedirectory')))
		{
			// Disable field on display.
		...
		if ($id != 0 && (!$user->authorise('tag.edit.file_type', 'com_servicedirectory.tag.' . (int) $id))
			|| ($id == 0 && !$user->authorise('tag.edit.file_type', 'com_servicedirectory')))
		{
			// Disable field on display.
	...
	protected function canDelete($record)
	{
		...
		return $this->getCurrentUser()->authorise('tag.delete', 'com_servicedirectory.tag.' . (int) $record->id);
	}
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('tag.edit.state', 'com_servicedirectory.tag.' . (int) $recordId);
		...
		return $user->authorise('tag.edit.state', 'com_servicedirectory');
	}
	...
	protected function allowEdit(array $data = [], string $key = 'id'): bool
	{
		...
		// Access check.
		$access = ($user->authorise('tag.access', 'com_servicedirectory.tag.' . (int) $recordId) && $user->authorise('tag.access', 'com_servicedirectory'));
		if (!$access)
		{
			return false;
		}
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('tag.edit', 'com_servicedirectory.tag.' . (int) $recordId);
			if (!$permission)
			{
				if ($user->authorise('tag.edit.own', 'com_servicedirectory.tag.' . $recordId))
				{
					...
					if ($ownerId == $user->id)
					{
						if ($user->authorise('tag.edit.own', 'com_servicedirectory'))
						{
							return true;
						}
					}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__servicedirectory_tag'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_servicedirectory.edit.tag.data', []);
		...
		$this->preprocessData('com_servicedirectory.tag', $data);
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		elseif (isset($values['published']) && !$this->canDo->get('tag.edit.state'))
		{
				$values['published'] = 0;
		}
		...
		while (!empty($pks))
		{
			...
			if (!$this->user->authorise('tag.edit', $contexts[$pk]))
			{
				// Not fatal error
	...
	protected function batchMove($values, $pks, $contexts)
	{
		...
		if (isset($values['published']) && !$this->canDo->get('tag.edit.state'))
		{
	...
	public function save($data)
	{
		...
		if (empty($data['guid']) && $data['id'] > 0)
		{
			...
			$data['guid'] = (string) GetHelper::var('tag', $data['id'], 'id', 'guid');
		}

		// Set the GUID if empty or not valid
		while (!GuidHelper::valid($data['guid'], "tag", $data['id']))
		{
			// must always be set
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null || empty($data['alias']))
			{
				...
				{
					$msg = Text::_('COM_SERVICEDIRECTORY_TAG_SAVE_WARNING');
				}
