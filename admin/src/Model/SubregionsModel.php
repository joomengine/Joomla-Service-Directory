...
/**
 * Subregions List Model
 *
 * @since  1.6
 */
class SubregionsModel extends ListModel
{
	/**
	...
	protected array $styles = [
		...
		'administrator/components/com_servicedirectory/assets/css/subregions.css'
 	];
	...
	public function getItems()
	{
		...
		if (UtilitiesArrayHelper::check($items))
		{
			...
			foreach ($items as $nr => &$item)
			{
				...
				$access = ($user->authorise('subregion.access', 'com_servicedirectory.subregion.' . (int) $item->id) && $user->authorise('subregion.access', 'com_servicedirectory'));
				if (!$access)
				{
					unset($items[$nr]);
					continue;
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__servicedirectory_subregion', 'a'));

		// From the servicedirectory_region table.
		$query->select($db->quoteName(['g.name','g.id'],['region_name','region_id']));
		$query->join('LEFT', $db->quoteName('#__servicedirectory_region', 'g') . ' ON (' . $db->quoteName('a.region') . ' = ' . $db->quoteName('g.guid') . ')');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.name LIKE '.$search.' OR a.region LIKE '.$search.' OR g.name LIKE '.$search.' OR a.wikidataid LIKE '.$search.')');
			}
		}
		...
		elseif (StringHelper::check($_region))
		{
	...
	protected function checkInNow(): void
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__servicedirectory_subregion'));
			...
			if ($db->getNumRows())
			{
				...
				// Check table.
				$query->update($db->quoteName('#__servicedirectory_subregion'))->set($fields)->where($conditions); 
