...
class LanguagesModel extends ListModel
{
	...
	protected array $styles = [
		...
		'administrator/components/com_servicedirectory/assets/css/languages.css'
 	];
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		if ($formSubmited)
		{
			$langtag = $input->post->get('langtag');
	...
	public function getItems()
	{
		...
		if (UtilitiesArrayHelper::check($items))
		{
			...
			foreach ($items as $nr => &$item)
			{
				...
				$access = ($user->authorise('language.access', 'com_servicedirectory.language.' . (int) $item->id) && $user->authorise('language.access', 'com_servicedirectory'));
				if (!$access)
				{
					unset($items[$nr]);
					continue;
	...
	protected function checkInNow(): void
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__servicedirectory_language'));
			...
			if ($db->getNumRows())
			{
				...
				// Check table.
				$query->update($db->quoteName('#__servicedirectory_language'))->set($fields)->where($conditions); 
