 ...
 */
namespace JoomService\Component\Servicedirectory\Administrator\View\Company_languages;
...
/**
 * Servicedirectory Html View class for the Company_languages
 *
 * @since  1.6
 */
...
class HtmlView extends BaseHtmlView
{
	...
	/**
	 * Company_languages view display method
	 *
	...
	public function display($tpl = null): void
	{
		...
		$this->canEdit = $this->canDo->get('company_language.edit');
		$this->canState = $this->canDo->get('company_language.edit.state');
		$this->canCreate = $this->canDo->get('company_language.create');
		$this->canDelete = $this->canDo->get('company_language.delete');
		$this->canBatch = ($this->canDo->get('company_language.batch') && $this->canDo->get('core.batch'));
	...
	{
		ToolbarHelper::title(Text::_('COM_SERVICEDIRECTORY_COMPANY_LANGUAGES'), 'lock');
		/** @var  Toolbar $toolbar */
		$toolbar = $this->getDocument()->getToolbar();
		if ($this->canCreate)
		{
			$toolbar->addNew('company_language.add');
		}
		...
		if (!$this->isEmptyState)
		{
			...
			if ($this->canEdit)
			{
				$childBar->edit('company_language.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('company_languages.publish')->listCheck(true);
				$childBar->unpublish('company_languages.unpublish')->listCheck(true);
				$childBar->archive('company_languages.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('company_languages.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('company_languages.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('company_languages.trash')->listCheck(true);
				}
			}
		}

		// set help url for this view if found
		$this->help_url = ServicedirectoryHelper::getHelpUrl('company_languages');
	...
	protected function getSortFields()
	{
		return array(
			...
			'g.name' => Text::_('COM_SERVICEDIRECTORY_COMPANY_LANGUAGE_LANGUAGE_LABEL'),
			'h.name' => Text::_('COM_SERVICEDIRECTORY_COMPANY_LANGUAGE_COMPANY_LABEL'),
