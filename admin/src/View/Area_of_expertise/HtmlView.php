 ...
 */
namespace JoomService\Component\Servicedirectory\Administrator\View\Area_of_expertise;
...
/**
 * Area_of_expertise Html View class
 *
 * @since  1.6
 */
...
class HtmlView extends BaseHtmlView
{
	...
	/**
	 * Area_of_expertise view display method
	 *
	...
	public function display($tpl = null): void
	{
		...
		$this->canDo = ServicedirectoryHelper::getActions('area_of_expertise', $this->item);
	...
	protected function addToolbar(): void
	{
		...
		ToolbarHelper::title( Text::_($isNew ? 'COM_SERVICEDIRECTORY_AREA_OF_EXPERTISE_NEW' : 'COM_SERVICEDIRECTORY_AREA_OF_EXPERTISE_EDIT'), 'pencil-2 article-add');
		// Built the actions for new and existing records.
		if (StringHelper::check($this->referral))
		{
			if ($this->canDo->get('area_of_expertise.create') && $isNew)
			{
				// We can create the record.
				ToolbarHelper::save('area_of_expertise.save', 'JTOOLBAR_SAVE');
			}
			elseif ($this->canDo->get('area_of_expertise.edit'))
			{
			...
			if ($isNew)
			{
				...
				ToolbarHelper::cancel('area_of_expertise.cancel', 'JTOOLBAR_CANCEL');
			}
			else
			{
				// We can close it.
				ToolbarHelper::cancel('area_of_expertise.cancel', 'JTOOLBAR_CLOSE');
			}
		}
		else
		{
			if ($isNew)
			{
				...
				if ($this->canDo->get('area_of_expertise.create'))
				{
					ToolbarHelper::apply('area_of_expertise.apply', 'JTOOLBAR_APPLY');
					ToolbarHelper::save('area_of_expertise.save', 'JTOOLBAR_SAVE');
					ToolbarHelper::custom('area_of_expertise.save2new', 'save-new.png', 'save-new_f2.png', 'JTOOLBAR_SAVE_AND_NEW', false);
				};
				ToolbarHelper::cancel('area_of_expertise.cancel', 'JTOOLBAR_CANCEL');
			}
			else
			{
				if ($this->canDo->get('area_of_expertise.edit'))
				{
				...
				$canVersion = ($this->canDo->get('core.version') && $this->canDo->get('area_of_expertise.version'));
				if ($this->state->params->get('save_history', 1) && $this->canDo->get('area_of_expertise.edit') && $canVersion)
				{
					ToolbarHelper::versions('com_servicedirectory.area_of_expertise', $this->item->id);
				}
				if ($this->canDo->get('area_of_expertise.create'))
				{
					ToolbarHelper::custom('area_of_expertise.save2copy', 'save-copy.png', 'save-copy_f2.png', 'JTOOLBAR_SAVE_AS_COPY', false);
				}
				ToolbarHelper::cancel('area_of_expertise.cancel', 'JTOOLBAR_CLOSE');
			}
		}
		...
		$this->help_url = ServicedirectoryHelper::getHelpUrl('area_of_expertise');
	...
	protected function _prepareDocument(): void
	{
		...
		$target ??= base64_encode('area_of_expertise');
