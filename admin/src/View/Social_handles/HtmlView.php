...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar(): void
	{
		...
		if ($this->canCreate)
		{
			$toolbar->addNew('social_handle.add');
		}
		...
		if (!$this->isEmptyState)
		{
			...
			if ($this->canEdit)
			{
				$childBar->edit('social_handle.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('social_handles.publish')->listCheck(true);
				$childBar->unpublish('social_handles.unpublish')->listCheck(true);
				$childBar->archive('social_handles.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('social_handles.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('social_handles.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('social_handles.trash')->listCheck(true);
				}
			}
		}
