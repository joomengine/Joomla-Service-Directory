 ...
 */
namespace JoomService\Component\Servicedirectory\Administrator\View\Ticket_comments;
...
/**
 * Servicedirectory Html View class for the Ticket_comments
 *
 * @since  1.6
 */
...
class HtmlView extends BaseHtmlView
{
	...
	/**
	 * Ticket_comments view display method
	 *
	...
	public function display($tpl = null): void
	{
		...
		$this->canEdit = $this->canDo->get('ticket_comment.edit');
		$this->canState = $this->canDo->get('ticket_comment.edit.state');
		$this->canCreate = $this->canDo->get('ticket_comment.create');
		$this->canDelete = $this->canDo->get('ticket_comment.delete');
		$this->canBatch = ($this->canDo->get('ticket_comment.batch') && $this->canDo->get('core.batch'));
	...
	{
		ToolbarHelper::title(Text::_('COM_SERVICEDIRECTORY_TICKET_COMMENTS'), 'comment');
		/** @var  Toolbar $toolbar */
		$toolbar = $this->getDocument()->getToolbar();
		if ($this->canCreate)
		{
			$toolbar->addNew('ticket_comment.add');
		}
		...
		if (!$this->isEmptyState)
		{
			...
			if ($this->canEdit)
			{
				$childBar->edit('ticket_comment.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('ticket_comments.publish')->listCheck(true);
				$childBar->unpublish('ticket_comments.unpublish')->listCheck(true);
				$childBar->archive('ticket_comments.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('ticket_comments.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('ticket_comments.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('ticket_comments.trash')->listCheck(true);
				}
			}
		}

		// set help url for this view if found
		$this->help_url = ServicedirectoryHelper::getHelpUrl('ticket_comments');
