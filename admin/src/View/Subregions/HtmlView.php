...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar(): void
	{
		...
		if (ArrayHelper::check($this->items))
		{
			...
			if ($this->canState)
			{
				ToolbarHelper::publishList('subregions.publish');
				ToolbarHelper::unpublishList('subregions.unpublish');
				ToolbarHelper::archiveList('subregions.archive');

				if ($this->canDo->get('core.admin'))
				{
					ToolbarHelper::checkin('subregions.checkin');
				}
			}

			if ($this->state->get('filter.published') == -2 && ($this->canState && $this->canDelete))
			{
				ToolbarHelper::deleteList('', 'subregions.delete', 'JTOOLBAR_EMPTY_TRASH');
			}
			elseif ($this->canState && $this->canDelete)
			{
				ToolbarHelper::trash('subregions.trash');
			}
		}
