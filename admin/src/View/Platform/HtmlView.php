 ...
 */
namespace JoomService\Component\Servicedirectory\Administrator\View\Platform;
...
class HtmlView extends BaseHtmlView
{
	...
	public function display($tpl = null): void
	{
		...
		$this->canDo = ServicedirectoryHelper::getActions('platform', $this->item);
	...
	protected function addToolbar(): void
	{
		...
		ToolbarHelper::title( Text::_($isNew ? 'COM_SERVICEDIRECTORY_PLATFORM_NEW' : 'COM_SERVICEDIRECTORY_PLATFORM_EDIT'), 'pencil-2 article-add');
		// Built the actions for new and existing records.
		if (StringHelper::check($this->referral))
		{
			if ($this->canDo->get('platform.create') && $isNew)
			{
				// We can create the record.
				ToolbarHelper::save('platform.save', 'JTOOLBAR_SAVE');
			}
			elseif ($this->canDo->get('platform.edit'))
			{
			...
			if ($isNew)
			{
				...
				ToolbarHelper::cancel('platform.cancel', 'JTOOLBAR_CANCEL');
			}
			else
			{
				// We can close it.
				ToolbarHelper::cancel('platform.cancel', 'JTOOLBAR_CLOSE');
			}
		}
		else
		{
			if ($isNew)
			{
				...
				if ($this->canDo->get('platform.create'))
				{
					ToolbarHelper::apply('platform.apply', 'JTOOLBAR_APPLY');
					ToolbarHelper::save('platform.save', 'JTOOLBAR_SAVE');
					ToolbarHelper::custom('platform.save2new', 'save-new.png', 'save-new_f2.png', 'JTOOLBAR_SAVE_AND_NEW', false);
				};
				ToolbarHelper::cancel('platform.cancel', 'JTOOLBAR_CANCEL');
			}
			else
			{
				if ($this->canDo->get('platform.edit'))
				{
				...
				$canVersion = ($this->canDo->get('core.version') && $this->canDo->get('platform.version'));
				if ($this->state->params->get('save_history', 1) && $this->canDo->get('platform.edit') && $canVersion)
				{
					ToolbarHelper::versions('com_servicedirectory.platform', $this->item->id);
				}
				if ($this->canDo->get('platform.create'))
				{
					ToolbarHelper::custom('platform.save2copy', 'save-copy.png', 'save-copy_f2.png', 'JTOOLBAR_SAVE_AS_COPY', false);
				}
				ToolbarHelper::cancel('platform.cancel', 'JTOOLBAR_CLOSE');
			}
		}
		...
		$this->help_url = ServicedirectoryHelper::getHelpUrl('platform');
