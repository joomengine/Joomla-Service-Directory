 ...
 */
namespace JoomService\Component\Servicedirectory\Administrator\View\Review_company_update;
...
/**
 * Review_company_update Html View class
 *
 * @since  1.6
 */
...
class HtmlView extends BaseHtmlView
{
	...
	/**
	 * Review_company_update view display method
	 *
	...
	public function display($tpl = null): void
	{
		...
		$this->canDo = Actions::get('review_company_update', $this->item);

		// Set the return
		$this->setReturn();

		// Get Linked view data
		$this->vvwsupport = $this->get('Vvwsupport');
	...
	protected function addToolbar(): void
	{
		...
		ToolbarHelper::title( Text::_($isNew ? 'COM_SERVICEDIRECTORY_REVIEW_COMPANY_UPDATE_NEW' : 'COM_SERVICEDIRECTORY_REVIEW_COMPANY_UPDATE_EDIT'), 'pencil-2 article-add');		// Built the actions for new and existing records.
		if (StringHelper::check($this->referral))
		{
			if ($this->canDo->get('core.create') && $isNew)
			{
				// Don't allow creation of new records
			}
			elseif ($this->canDo->get('core.edit'))
			{
				// We can save the record.
				ToolbarHelper::save('review_company_update.save', 'JTOOLBAR_SAVE');
			}
			if ($isNew)
			{
				// Do not create but cancel.
				ToolbarHelper::cancel('review_company_update.cancel', 'JTOOLBAR_CANCEL');
			}
			else
			{
				// We can close it.
				ToolbarHelper::cancel('review_company_update.cancel', 'JTOOLBAR_CLOSE');
			}
		}
		else
		{
			...
			else
			{
				if ($this->canDo->get('core.edit'))
				{
					...
					ToolbarHelper::apply('review_company_update.apply', 'JTOOLBAR_APPLY');
					ToolbarHelper::save('review_company_update.save', 'JTOOLBAR_SAVE');
				}
				ToolbarHelper::cancel('review_company_update.cancel', 'JTOOLBAR_CLOSE');
			}
	...
	protected function addModalToolbar()
	{
		...
		ToolbarHelper::title( Text::_($isNew ? 'COM_SERVICEDIRECTORY_REVIEW_COMPANY_UPDATE_NEW' : 'COM_SERVICEDIRECTORY_REVIEW_COMPANY_UPDATE_EDIT'), 'pencil-2 article-add');
		...
		else
		{
			if ($isNew)
			{
				...
				if ($this->canDo->get('core.create'))
				{
					ToolbarHelper::apply('review_company_update.apply', 'JTOOLBAR_APPLY');
					ToolbarHelper::save('review_company_update.save', 'JTOOLBAR_SAVE');
					ToolbarHelper::custom('review_company_update.save2new', 'save-new.png', 'save-new_f2.png', 'JTOOLBAR_SAVE_AND_NEW', false);
				};
				ToolbarHelper::cancel('review_company_update.cancel', 'JTOOLBAR_CANCEL');
			}
			else
	...
	protected function _prepareDocument(): void
	{
		...
		Html::_('stylesheet', 'media/com_servicedirectory/footable-v2/css/footable.core.min.css', ['version' => 'auto']);

		// Use the Metro Style
		if (!isset($this->fooTableStyle) || 0 == $this->fooTableStyle)
		{
			Html::_('stylesheet', 'media/com_servicedirectory/footable-v2/css/footable.metro.min.css', ['version' => 'auto']);
		}
		// Use the Legacy Style.
		elseif (isset($this->fooTableStyle) && 1 == $this->fooTableStyle)
		{
			Html::_('stylesheet', 'media/com_servicedirectory/footable-v2/css/footable.standalone.min.css', ['version' => 'auto']);
		}

		// Add the JavaScript for Footable
		Html::_('script', 'media/com_servicedirectory/footable-v2/js/footable.js', ['version' => 'auto']);
		Html::_('script', 'media/com_servicedirectory/footable-v2/js/footable.sort.js', ['version' => 'auto']);
		Html::_('script', 'media/com_servicedirectory/footable-v2/js/footable.filter.js', ['version' => 'auto']);
		Html::_('script', 'media/com_servicedirectory/footable-v2/js/footable.paginate.js', ['version' => 'auto']);
		...
		$target ??= base64_encode('review_company_update');
		...
		Text::script('COM_SERVICEDIRECTORY_SELECT_A_COUNTRY_FIRST');
		Text::script('COM_SERVICEDIRECTORY_SELECT_A_STATE');
		Text::script('COM_SERVICEDIRECTORY_SELECT_A_STATE_FIRST');
		Text::script('COM_SERVICEDIRECTORY_SELECT_A_CITY');
		$cascadingSelectManagerConfig = (object) [
			...
			'chain' => [
				(object) [
					'name' => 'country',
					'id' => '__country',
					'placeholder' => (object) ['ready' => 'COM_SERVICEDIRECTORY_SELECT_A_COUNTRY']
				],
				(object) [
					'name' => 'state',
					'id' => '__state',
					'endpoint' => (object) [
						'url' => "{$url}index.php?option=com_servicedirectory&task=ajax.getCountryStates&format=json&raw=true&{$token}=1",
						'basedOn' => 'country',
						'paramKey' => 'country'
					],
					'placeholder' => (object) ['initial' => 'COM_SERVICEDIRECTORY_SELECT_A_COUNTRY_FIRST', 'ready' => 'COM_SERVICEDIRECTORY_SELECT_A_STATE']
				],
				(object) [
					'name' => 'city',
					'id' => '__city',
					'endpoint' => (object) [
						'url' => "{$url}index.php?option=com_servicedirectory&task=ajax.getStateCities&format=json&raw=true&{$token}=1",
						'basedOn' => 'state',
						'paramKey' => 'state'
					],
					'placeholder' => (object) ['initial' => 'COM_SERVICEDIRECTORY_SELECT_A_STATE_FIRST', 'ready' => 'COM_SERVICEDIRECTORY_SELECT_A_CITY']
				]
			]
		];
