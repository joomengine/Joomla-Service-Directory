 ...
 */
namespace JoomService\Component\Servicedirectory\Administrator\View\Address;
...
class HtmlView extends BaseHtmlView
{
	...
	public function display($tpl = null): void
	{
		...
		$this->canDo = ServicedirectoryHelper::getActions('address', $this->item);
	...
	protected function addToolbar(): void
	{
		...
		ToolbarHelper::title( Text::_($isNew ? 'COM_SERVICEDIRECTORY_ADDRESS_NEW' : 'COM_SERVICEDIRECTORY_ADDRESS_EDIT'), 'pencil-2 article-add');
		// Built the actions for new and existing records.
		if (StringHelper::check($this->referral))
		{
			if ($this->canDo->get('address.create') && $isNew)
			{
				// We can create the record.
				ToolbarHelper::save('address.save', 'JTOOLBAR_SAVE');
			}
			elseif ($this->canDo->get('address.edit'))
			{
		...
		else
		{
			if ($isNew)
			{
				...
				if ($this->canDo->get('address.create'))
				{
			...
			{
				if ($this->canDo->get('address.edit'))
				{
				...
				$canVersion = ($this->canDo->get('core.version') && $this->canDo->get('address.version'));
				if ($this->state->params->get('save_history', 1) && $this->canDo->get('address.edit') && $canVersion)
				{
					ToolbarHelper::versions('com_servicedirectory.address', $this->item->id);
				}
				if ($this->canDo->get('address.create'))
				{
		...
		$this->help_url = ServicedirectoryHelper::getHelpUrl('address');
