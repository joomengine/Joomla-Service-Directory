 ...
 */
namespace JoomService\Component\Servicedirectory\Administrator\View\States;
...
/**
 * Servicedirectory Html View class for the States
 *
 * @since  1.6
 */
...
class HtmlView extends BaseHtmlView
{
	...
	public function display($tpl = null): void
	{
		...
		$this->canEdit = $this->canDo->get('state.edit');
		$this->canState = $this->canDo->get('state.edit.state');
		$this->canCreate = $this->canDo->get('state.create');
		$this->canDelete = $this->canDo->get('state.delete');
		$this->canBatch = ($this->canDo->get('state.batch') && $this->canDo->get('core.batch'));
	...
	{
		ToolbarHelper::title(Text::_('COM_SERVICEDIRECTORY_STATES'), 'flag');
		/** @var  Toolbar $toolbar */
		$toolbar = $this->getDocument()->getToolbar();
		if ($this->canCreate)
		{
			$toolbar->addNew('state.add');
		}
		...
		if (!$this->isEmptyState)
		{
			...
			if ($this->canEdit)
			{
				$childBar->edit('state.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('states.publish')->listCheck(true);
				$childBar->unpublish('states.unpublish')->listCheck(true);
				$childBar->archive('states.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('states.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('states.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('states.trash')->listCheck(true);
				}
			}
		}

		// set help url for this view if found
		$this->help_url = ServicedirectoryHelper::getHelpUrl('states');
	...
	protected function getSortFields()
	{
		return array(
			...
			'a.name' => Text::_('COM_SERVICEDIRECTORY_STATE_NAME_LABEL'),
			'g.name' => Text::_('COM_SERVICEDIRECTORY_STATE_COUNTRY_LABEL'),
			'a.type' => Text::_('COM_SERVICEDIRECTORY_STATE_TYPE_LABEL'),
