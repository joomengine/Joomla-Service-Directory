 ...
 */
namespace JoomService\Component\Servicedirectory\Administrator\View\Ticket_comment;
...
/**
 * Ticket_comment Html View class
 *
 * @since  1.6
 */
...
class HtmlView extends BaseHtmlView
{
	...
	/**
	 * Ticket_comment view display method
	 *
	...
	public function display($tpl = null): void
	{
		...
		$this->canDo = ServicedirectoryHelper::getActions('ticket_comment', $this->item);
	...
	protected function addToolbar(): void
	{
		...
		ToolbarHelper::title( Text::_($isNew ? 'COM_SERVICEDIRECTORY_TICKET_COMMENT_NEW' : 'COM_SERVICEDIRECTORY_TICKET_COMMENT_EDIT'), 'pencil-2 article-add');
		// Built the actions for new and existing records.
		if (StringHelper::check($this->referral))
		{
			if ($this->canDo->get('ticket_comment.create') && $isNew)
			{
				// We can create the record.
				ToolbarHelper::save('ticket_comment.save', 'JTOOLBAR_SAVE');
			}
			elseif ($this->canDo->get('ticket_comment.edit'))
			{
			...
			if ($isNew)
			{
				...
				ToolbarHelper::cancel('ticket_comment.cancel', 'JTOOLBAR_CANCEL');
			}
			else
			{
				// We can close it.
				ToolbarHelper::cancel('ticket_comment.cancel', 'JTOOLBAR_CLOSE');
			}
		}
		else
		{
			if ($isNew)
			{
				...
				if ($this->canDo->get('ticket_comment.create'))
				{
					ToolbarHelper::apply('ticket_comment.apply', 'JTOOLBAR_APPLY');
					ToolbarHelper::save('ticket_comment.save', 'JTOOLBAR_SAVE');
					ToolbarHelper::custom('ticket_comment.save2new', 'save-new.png', 'save-new_f2.png', 'JTOOLBAR_SAVE_AND_NEW', false);
				};
				ToolbarHelper::cancel('ticket_comment.cancel', 'JTOOLBAR_CANCEL');
			}
			else
			{
				if ($this->canDo->get('ticket_comment.edit'))
				{
				...
				$canVersion = ($this->canDo->get('core.version') && $this->canDo->get('ticket_comment.version'));
				if ($this->state->params->get('save_history', 1) && $this->canDo->get('ticket_comment.edit') && $canVersion)
				{
					ToolbarHelper::versions('com_servicedirectory.ticket_comment', $this->item->id);
				}
				if ($this->canDo->get('ticket_comment.create'))
				{
					ToolbarHelper::custom('ticket_comment.save2copy', 'save-copy.png', 'save-copy_f2.png', 'JTOOLBAR_SAVE_AS_COPY', false);
				}
				ToolbarHelper::cancel('ticket_comment.cancel', 'JTOOLBAR_CLOSE');
			}
		}
		...
		$this->help_url = ServicedirectoryHelper::getHelpUrl('ticket_comment');
	...
	protected function _prepareDocument(): void
	{
		...
		$target ??= base64_encode('ticket_comment');
