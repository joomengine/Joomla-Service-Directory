...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar(): void
	{
		...
		if (ArrayHelper::check($this->items))
		{
			...
			if ($this->canState)
			{
				ToolbarHelper::publishList('regions.publish');
				ToolbarHelper::unpublishList('regions.unpublish');
				ToolbarHelper::archiveList('regions.archive');

				if ($this->canDo->get('core.admin'))
				{
					ToolbarHelper::checkin('regions.checkin');
				}
			}

			if ($this->state->get('filter.published') == -2 && ($this->canState && $this->canDelete))
			{
				ToolbarHelper::deleteList('', 'regions.delete', 'JTOOLBAR_EMPTY_TRASH');
			}
			elseif ($this->canState && $this->canDelete)
			{
				ToolbarHelper::trash('regions.trash');
			}
		}
