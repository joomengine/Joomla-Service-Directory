 ...
 */
namespace JoomService\Component\Servicedirectory\Administrator\View\Regions;
...
/**
 * Servicedirectory Html View class for the Regions
 *
 * @since  1.6
 */
...
class HtmlView extends BaseHtmlView
{
	...
	public function display($tpl = null): void
	{
		...
		$this->canBatch = ($this->canDo->get('region.batch') && $this->canDo->get('core.batch'));
	...
	{
		ToolbarHelper::title(Text::_('COM_SERVICEDIRECTORY_REGIONS'), 'arrow-down-2');
		/** @var  Toolbar $toolbar */
		$toolbar = $this->getDocument()->getToolbar();
		if ($this->canCreate)
		{
			$toolbar->addNew('region.add');
		}
		...
		if (!$this->isEmptyState)
		{
			...
			if ($this->canEdit)
			{
				$childBar->edit('region.edit')->listCheck(true);
			}
			...
			if ($this->canState)
			{
				...
				$childBar->archive('regions.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('regions.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('regions.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('regions.trash')->listCheck(true);
				}
			}
		}

		// set help url for this view if found
		$this->help_url = ServicedirectoryHelper::getHelpUrl('regions');
	...
	protected function getSortFields()
	{
		return array(
			...
			'a.name' => Text::_('COM_SERVICEDIRECTORY_REGION_NAME_LABEL'),
