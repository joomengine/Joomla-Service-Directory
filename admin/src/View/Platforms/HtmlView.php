...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar(): void
	{
		...
		if (ArrayHelper::check($this->items))
		{
			if ($this->canEdit)
			{
				ToolbarHelper::editList('platform.edit');
			}

			if ($this->canState)
			{
				ToolbarHelper::publishList('platforms.publish');
				ToolbarHelper::unpublishList('platforms.unpublish');
				ToolbarHelper::archiveList('platforms.archive');

				if ($this->canDo->get('core.admin'))
				{
					ToolbarHelper::checkin('platforms.checkin');
				}
			}

			if ($this->state->get('filter.published') == -2 && ($this->canState && $this->canDelete))
			{
				ToolbarHelper::deleteList('', 'platforms.delete', 'JTOOLBAR_EMPTY_TRASH');
			}
			elseif ($this->canState && $this->canDelete)
			{
				ToolbarHelper::trash('platforms.trash');
			}
		}
