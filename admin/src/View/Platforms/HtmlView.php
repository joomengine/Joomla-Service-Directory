 ...
 */
namespace JoomService\Component\Servicedirectory\Administrator\View\Platforms;
...
/**
 * Servicedirectory Html View class for the Platforms
 *
 * @since  1.6
 */
...
class HtmlView extends BaseHtmlView
{
	...
	public function display($tpl = null): void
	{
		...
		$this->canEdit = $this->canDo->get('platform.edit');
		$this->canState = $this->canDo->get('platform.edit.state');
		$this->canCreate = $this->canDo->get('platform.create');
		$this->canDelete = $this->canDo->get('platform.delete');
		$this->canBatch = ($this->canDo->get('platform.batch') && $this->canDo->get('core.batch'));
	...
	{
		ToolbarHelper::title(Text::_('COM_SERVICEDIRECTORY_PLATFORMS'), 'wifi');
		/** @var  Toolbar $toolbar */
		$toolbar = $this->getDocument()->getToolbar();
		if ($this->canCreate)
		{
			$toolbar->addNew('platform.add');
		}
		...
		if (!$this->isEmptyState)
		{
			...
			if ($this->canEdit)
			{
				$childBar->edit('platform.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('platforms.publish')->listCheck(true);
				$childBar->unpublish('platforms.unpublish')->listCheck(true);
				$childBar->archive('platforms.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('platforms.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('platforms.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('platforms.trash')->listCheck(true);
				}
			}
		}

		// set help url for this view if found
		$this->help_url = ServicedirectoryHelper::getHelpUrl('platforms');
	...
	protected function getSortFields()
	{
		return array(
			...
			'a.name' => Text::_('COM_SERVICEDIRECTORY_PLATFORM_NAME_LABEL'),
