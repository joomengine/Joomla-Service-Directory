 ...
 */
namespace JoomService\Component\Servicedirectory\Administrator\View\Cities;
...
/**
 * Servicedirectory Html View class for the Cities
 *
 * @since  1.6
 */
...
class HtmlView extends BaseHtmlView
{
	...
	public function display($tpl = null): void
	{
		...
		$this->canBatch = ($this->canDo->get('city.batch') && $this->canDo->get('core.batch'));
	...
	{
		ToolbarHelper::title(Text::_('COM_SERVICEDIRECTORY_CITIES'), 'home');

		if ($this->canCreate)
		{
			ToolbarHelper::addNew('city.add');
		}
		...
		if (ArrayHelper::check($this->items))
		{
			if ($this->canEdit)
			{
				ToolbarHelper::editList('city.edit');
			}

			if ($this->canState)
			{
				ToolbarHelper::publishList('cities.publish');
				ToolbarHelper::unpublishList('cities.unpublish');
				ToolbarHelper::archiveList('cities.archive');

				if ($this->canDo->get('core.admin'))
				{
					ToolbarHelper::checkin('cities.checkin');
				}
			}

			if ($this->state->get('filter.published') == -2 && ($this->canState && $this->canDelete))
			{
				ToolbarHelper::deleteList('', 'cities.delete', 'JTOOLBAR_EMPTY_TRASH');
			}
			elseif ($this->canState && $this->canDelete)
			{
				ToolbarHelper::trash('cities.trash');
			}
		}

		// set help url for this view if found
		$this->help_url = ServicedirectoryHelper::getHelpUrl('cities');
	...
	protected function getSortFields()
	{
		return array(
			...
			'a.name' => Text::_('COM_SERVICEDIRECTORY_CITY_NAME_LABEL'),
			'g.name' => Text::_('COM_SERVICEDIRECTORY_CITY_STATE_LABEL'),
			'a.latitude' => Text::_('COM_SERVICEDIRECTORY_CITY_LATITUDE_LABEL'),
			'a.longitude' => Text::_('COM_SERVICEDIRECTORY_CITY_LONGITUDE_LABEL'),
