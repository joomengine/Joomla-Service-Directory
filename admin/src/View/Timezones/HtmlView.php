 ...
 */
namespace JoomService\Component\Servicedirectory\Administrator\View\Timezones;
...
/**
 * Servicedirectory Html View class for the Timezones
 *
 * @since  1.6
 */
...
class HtmlView extends BaseHtmlView
{
	...
	public function display($tpl = null): void
	{
		...
		$this->canEdit = $this->canDo->get('timezone.edit');
		$this->canState = $this->canDo->get('timezone.edit.state');
		$this->canCreate = $this->canDo->get('timezone.create');
		$this->canDelete = $this->canDo->get('timezone.delete');
		$this->canBatch = ($this->canDo->get('timezone.batch') && $this->canDo->get('core.batch'));
	...
	{
		ToolbarHelper::title(Text::_('COM_SERVICEDIRECTORY_TIMEZONES'), 'clock');
		/** @var  Toolbar $toolbar */
		$toolbar = $this->getDocument()->getToolbar();
		if ($this->canCreate)
		{
			$toolbar->addNew('timezone.add');
		}
		...
		if (!$this->isEmptyState)
		{
			...
			if ($this->canEdit)
			{
				$childBar->edit('timezone.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('timezones.publish')->listCheck(true);
				$childBar->unpublish('timezones.unpublish')->listCheck(true);
				$childBar->archive('timezones.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('timezones.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('timezones.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('timezones.trash')->listCheck(true);
				}
			}
		}

		// set help url for this view if found
		$this->help_url = ServicedirectoryHelper::getHelpUrl('timezones');
	...
	protected function getSortFields()
	{
		return array(
			...
			'a.timezone_name' => Text::_('COM_SERVICEDIRECTORY_TIMEZONE_TIMEZONE_NAME_LABEL'),
			'a.timezone_identifier' => Text::_('COM_SERVICEDIRECTORY_TIMEZONE_TIMEZONE_IDENTIFIER_LABEL'),
			'a.gmt_offset_name' => Text::_('COM_SERVICEDIRECTORY_TIMEZONE_GMT_OFFSET_NAME_LABEL'),
			'g.name' => Text::_('COM_SERVICEDIRECTORY_TIMEZONE_COUNTRY_LABEL'),
