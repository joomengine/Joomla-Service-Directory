 ...
 */
namespace JoomService\Component\Servicedirectory\Administrator\View\Category;
...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar(): void
	{
		...
		ToolbarHelper::title( Text::_($isNew ? 'COM_SERVICEDIRECTORY_CATEGORY_NEW' : 'COM_SERVICEDIRECTORY_CATEGORY_EDIT'), 'pencil-2 article-add');
		// Built the actions for new and existing records.
		if (StringHelper::check($this->referral))
		{
			if ($this->canDo->get('category.create') && $isNew)
			{
				// We can create the record.
				ToolbarHelper::save('category.save', 'JTOOLBAR_SAVE');
			}
			elseif ($this->canDo->get('category.edit'))
			{
		...
		else
		{
			if ($isNew)
			{
				...
				if ($this->canDo->get('category.create'))
				{
			...
			{
				if ($this->canDo->get('category.edit'))
				{
				...
				$canVersion = ($this->canDo->get('core.version') && $this->canDo->get('category.version'));
				if ($this->state->params->get('save_history', 1) && $this->canDo->get('category.edit') && $canVersion)
				{
					ToolbarHelper::versions('com_servicedirectory.category', $this->item->id);
				}
				if ($this->canDo->get('category.create'))
				{
		...
		$this->help_url = ServicedirectoryHelper::getHelpUrl('category');
