 ...
 */
namespace JoomService\Component\Servicedirectory\Administrator\View\Files;
...
/**
 * Servicedirectory Html View class for the Files
 *
 * @since  1.6
 */
...
class HtmlView extends BaseHtmlView
{
	...
	public function display($tpl = null): void
	{
		...
		$this->canEdit = $this->canDo->get('file.edit');
		$this->canState = $this->canDo->get('file.edit.state');
		$this->canCreate = $this->canDo->get('file.create');
		$this->canDelete = $this->canDo->get('file.delete');
		$this->canBatch = ($this->canDo->get('file.batch') && $this->canDo->get('core.batch'));
	...
	{
		ToolbarHelper::title(Text::_('COM_SERVICEDIRECTORY_FILES'), 'archive');
		/** @var  Toolbar $toolbar */
		...
		if (!$this->isEmptyState)
		{
			...
			if ($this->canEdit)
			{
				$childBar->edit('file.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('files.publish')->listCheck(true);
				$childBar->unpublish('files.unpublish')->listCheck(true);
				$childBar->archive('files.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('files.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('files.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('files.trash')->listCheck(true);
				}
			}
		}

		// set help url for this view if found
		$this->help_url = ServicedirectoryHelper::getHelpUrl('files');
	...
	protected function getSortFields()
	{
		return array(
			...
			'a.name' => Text::_('COM_SERVICEDIRECTORY_FILE_NAME_LABEL'),
			'g.name' => Text::_('COM_SERVICEDIRECTORY_FILE_FILE_TYPE_LABEL'),
			'a.size' => Text::_('COM_SERVICEDIRECTORY_FILE_SIZE_LABEL'),
			'a.entity_type' => Text::_('COM_SERVICEDIRECTORY_FILE_ENTITY_TYPE_LABEL'),
