 ...
 */
namespace JoomService\Component\Servicedirectory\Administrator\View\Portfolios;
...
/**
 * Servicedirectory Html View class for the Portfolios
 *
 * @since  1.6
 */
...
class HtmlView extends BaseHtmlView
{
	...
	/**
	 * Portfolios view display method
	 *
	...
	public function display($tpl = null): void
	{
		...
		$this->canEdit = $this->canDo->get('portfolio.edit');
		$this->canState = $this->canDo->get('portfolio.edit.state');
		$this->canCreate = $this->canDo->get('portfolio.create');
		$this->canDelete = $this->canDo->get('portfolio.delete');
		$this->canBatch = ($this->canDo->get('portfolio.batch') && $this->canDo->get('core.batch'));
	...
	{
		ToolbarHelper::title(Text::_('COM_SERVICEDIRECTORY_PORTFOLIOS'), 'camera-2 / video');
		/** @var  Toolbar $toolbar */
		...
		if (!$this->isEmptyState)
		{
			...
			if ($this->canEdit)
			{
				$childBar->edit('portfolio.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('portfolios.publish')->listCheck(true);
				$childBar->unpublish('portfolios.unpublish')->listCheck(true);
				$childBar->archive('portfolios.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('portfolios.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('portfolios.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('portfolios.trash')->listCheck(true);
				}
			}
		}

		// set help url for this view if found
		$this->help_url = ServicedirectoryHelper::getHelpUrl('portfolios');
	...
	protected function getSortFields()
	{
		return array(
			...
			'a.project_title' => Text::_('COM_SERVICEDIRECTORY_PORTFOLIO_PROJECT_TITLE_LABEL'),
			'g.name' => Text::_('COM_SERVICEDIRECTORY_PORTFOLIO_COMPANY_LABEL'),
			'a.client_name' => Text::_('COM_SERVICEDIRECTORY_PORTFOLIO_CLIENT_NAME_LABEL'),
			'a.target_industry' => Text::_('COM_SERVICEDIRECTORY_PORTFOLIO_TARGET_INDUSTRY_LABEL'),
