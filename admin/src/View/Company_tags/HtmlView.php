 ...
 */
namespace JoomService\Component\Servicedirectory\Administrator\View\Company_tags;
...
/**
 * Servicedirectory Html View class for the Company_tags
 *
 * @since  1.6
 */
...
class HtmlView extends BaseHtmlView
{
	...
	/**
	 * Company_tags view display method
	 *
	...
	public function display($tpl = null): void
	{
		...
		$this->canEdit = $this->canDo->get('company_tag.edit');
		$this->canState = $this->canDo->get('company_tag.edit.state');
		$this->canCreate = $this->canDo->get('company_tag.create');
		$this->canDelete = $this->canDo->get('company_tag.delete');
		$this->canBatch = ($this->canDo->get('company_tag.batch') && $this->canDo->get('core.batch'));
	...
	{
		ToolbarHelper::title(Text::_('COM_SERVICEDIRECTORY_COMPANY_TAGS'), 'bars');
		/** @var  Toolbar $toolbar */
		$toolbar = $this->getDocument()->getToolbar();
		if ($this->canCreate)
		{
			$toolbar->addNew('company_tag.add');
		}
		...
		if (!$this->isEmptyState)
		{
			...
			if ($this->canEdit)
			{
				$childBar->edit('company_tag.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('company_tags.publish')->listCheck(true);
				$childBar->unpublish('company_tags.unpublish')->listCheck(true);
				$childBar->archive('company_tags.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('company_tags.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('company_tags.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('company_tags.trash')->listCheck(true);
				}
			}
		}

		// set help url for this view if found
		$this->help_url = ServicedirectoryHelper::getHelpUrl('company_tags');
	...
	protected function getSortFields()
	{
		return array(
			...
			'g.name' => Text::_('COM_SERVICEDIRECTORY_COMPANY_TAG_TAG_LABEL'),
			'h.name' => Text::_('COM_SERVICEDIRECTORY_COMPANY_TAG_COMPANY_LABEL'),
