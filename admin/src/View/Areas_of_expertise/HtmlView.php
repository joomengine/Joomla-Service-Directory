 ...
 */
namespace JoomService\Component\Servicedirectory\Administrator\View\Areas_of_expertise;
...
/**
 * Servicedirectory Html View class for the Areas_of_expertise
 *
 * @since  1.6
 */
...
class HtmlView extends BaseHtmlView
{
	...
	/**
	 * Areas_of_expertise view display method
	 *
	...
	public function display($tpl = null): void
	{
		...
		$this->canEdit = $this->canDo->get('area_of_expertise.edit');
		$this->canState = $this->canDo->get('area_of_expertise.edit.state');
		$this->canCreate = $this->canDo->get('area_of_expertise.create');
		$this->canDelete = $this->canDo->get('area_of_expertise.delete');
		$this->canBatch = ($this->canDo->get('area_of_expertise.batch') && $this->canDo->get('core.batch'));
	...
	{
		ToolbarHelper::title(Text::_('COM_SERVICEDIRECTORY_AREAS_OF_EXPERTISE'), 'comment');
		/** @var  Toolbar $toolbar */
		$toolbar = $this->getDocument()->getToolbar();
		if ($this->canCreate)
		{
			$toolbar->addNew('area_of_expertise.add');
		}
		...
		if (!$this->isEmptyState)
		{
			...
			if ($this->canEdit)
			{
				$childBar->edit('area_of_expertise.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('areas_of_expertise.publish')->listCheck(true);
				$childBar->unpublish('areas_of_expertise.unpublish')->listCheck(true);
				$childBar->archive('areas_of_expertise.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('areas_of_expertise.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('areas_of_expertise.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('areas_of_expertise.trash')->listCheck(true);
				}
			}
		}

		// set help url for this view if found
		$this->help_url = ServicedirectoryHelper::getHelpUrl('areas_of_expertise');
	...
	protected function getSortFields()
	{
		return array(
			...
			'a.name' => Text::_('COM_SERVICEDIRECTORY_AREA_OF_EXPERTISE_NAME_LABEL'),
			'a.description' => Text::_('COM_SERVICEDIRECTORY_AREA_OF_EXPERTISE_DESCRIPTION_LABEL'),
