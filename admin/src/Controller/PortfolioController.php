...
class PortfolioController extends FormController
{
	 ...
	 * @since  1.6
	 */
	protected $text_prefix = 'COM_SERVICEDIRECTORY_PORTFOLIO';
	 ...
	 * @since  1.6
	 */
	protected $view_list = 'portfolios';
	...
	protected function allowAdd($data = [])
	{
		...
		// Access check.
		$access = $user->authorise('portfolio.access', 'com_servicedirectory');
		if (!$access)
		{
			return false;
		}
		...
		return $user->authorise('portfolio.create', $this->option);
	}
	...
	public function batch($model = null)
	{
		...
		// Preset the redirect
		$this->setRedirect(Route::_('index.php?option=com_servicedirectory&view=portfolios' . $this->getRedirectToListAppend(), false));
	...
	protected function postSaveHook(BaseDatabaseModel $model, $validData = [])
	{
		...
		{
			// When editing in modal then redirect to modalreturn layout
			$id = $model->getState('portfolio.id', '');
			$return = 'index.php?option=' . $this->option . '&view=' . $this->view_item . $this->getRedirectToItemAppend($id)
