...
class CountryController extends FormController
{
	 ...
	 * @since  1.6
	 */
	protected $text_prefix = 'COM_SERVICEDIRECTORY_COUNTRY';
	...
	public function edit($key = null, $urlVar = null)
	{
		...
		{
			$value = GuidHelper::item($guid, 'country', 'a.' . $key, 'servicedirectory');
		}
	...
	protected function allowAdd($data = [])
	{
		...
		// Access check.
		$access = $user->authorise('country.access', 'com_servicedirectory');
		if (!$access)
		{
			return false;
		}
	...
	protected function allowEdit($data = [], $key = 'id')
	{
		...
		// Access check.
		$access = ($user->authorise('country.access', 'com_servicedirectory.country.' . (int) $recordId) && $user->authorise('country.access', 'com_servicedirectory'));
		if (!$access)
		{
			return false;
		}
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('country.edit', 'com_servicedirectory.country.' . (int) $recordId);
			if (!$permission)
			{
				if ($user->authorise('country.edit.own', 'com_servicedirectory.country.' . $recordId))
				{
					...
					if ($ownerId == $user->id)
					{
						if ($user->authorise('country.edit.own', 'com_servicedirectory'))
						{
							return true;
						}
					}
	...
	public function batch($model = null)
	{
		...
		// Preset the redirect
		$this->setRedirect(Route::_('index.php?option=com_servicedirectory&view=countries' . $this->getRedirectToListAppend(), false));
