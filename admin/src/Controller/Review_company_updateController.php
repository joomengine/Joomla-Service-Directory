...
/**
 * Review_company_update Form Controller
 *
 * @since  1.6
 */
class Review_company_updateController extends FormController
{
	 ...
	 * @since  1.6
	 */
	protected $text_prefix = 'COM_SERVICEDIRECTORY_REVIEW_COMPANY_UPDATE';
	 ...
	 * @since  1.6
	 */
	protected $view_list = 'review_company_updates';
	...
	public function edit($key = null, $urlVar = null)
	{
		...
		{
			$value = GuidHelper::item($guid, 'review_company_update', 'a.' . $key, 'servicedirectory');
		}
	...
	protected function allowAdd($data = [])
	{
		...
		// Access check.
		$access = $user->authorise('review_company_update.access', 'com_servicedirectory');
		if (!$access)
		{
			return false;
		}
	...
	public function batch($model = null)
	{
		...
		// Set the model
		$model = $this->getModel('Review_company_update', '', []);

		// Preset the redirect
		$this->setRedirect(Route::_('index.php?option=com_servicedirectory&view=review_company_updates' . $this->getRedirectToListAppend(), false));
	...
	protected function postSaveHook(BaseDatabaseModel $model, $validData = [])
	{
		...
		{
			// When editing in modal then redirect to modalreturn layout
			$id = $model->getState('review_company_update.id', '');
			$return = 'index.php?option=' . $this->option . '&view=' . $this->view_item . $this->getRedirectToItemAppend($id)
