...
/**
 * Company_languages Table class
 *
 * @since  1.6
 */
class Company_languageTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_servicedirectory.company_language';
		...
		parent::__construct('#__servicedirectory_company_language', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Company_language table.
	 *
	...
	public function store($updateNulls = true)
	{
		...
		{
			// New company_language. A company_language created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_SERVICEDIRECTORY_COMPANY_LANGUAGE_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_SERVICEDIRECTORY_COMPANY_LANGUAGE_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_servicedirectory.company_language.'.$this->id);
			$this->setRules($rules);
		}
