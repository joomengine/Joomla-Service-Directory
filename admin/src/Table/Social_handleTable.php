...
/**
 * Social_handles Table class
 *
 * @since  1.6
 */
class Social_handleTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_servicedirectory.social_handle';
		...
		parent::__construct('#__servicedirectory_social_handle', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Social_handle table.
	 *
	...
	public function store($updateNulls = true)
	{
		...
		{
			// New social_handle. A social_handle created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_SERVICEDIRECTORY_SOCIAL_HANDLE_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_SERVICEDIRECTORY_SOCIAL_HANDLE_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_servicedirectory.social_handle.'.$this->id);
			$this->setRules($rules);
		}
