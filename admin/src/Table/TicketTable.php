 ...
 * @since  1.6
 */
class TicketTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_servicedirectory.ticket';
		...
		parent::__construct('#__servicedirectory_ticket', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Ticket table.
	 *
	...
	public function store($updateNulls = true)
	{
		...
		{
			// New ticket. A ticket created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_SERVICEDIRECTORY_TICKET_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_SERVICEDIRECTORY_TICKET_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_servicedirectory.ticket.'.$this->id);
			$this->setRules($rules);
		}
