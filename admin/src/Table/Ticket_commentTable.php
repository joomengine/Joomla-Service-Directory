...
/**
 * Ticket_comments Table class
 *
 * @since  1.6
 */
class Ticket_commentTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_servicedirectory.ticket_comment';
		...
		parent::__construct('#__servicedirectory_ticket_comment', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Ticket_comment table.
	 *
	...
	public function store($updateNulls = true)
	{
		...
		{
			// New ticket_comment. A ticket_comment created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_SERVICEDIRECTORY_TICKET_COMMENT_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_SERVICEDIRECTORY_TICKET_COMMENT_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_servicedirectory.ticket_comment.'.$this->id);
			$this->setRules($rules);
		}
