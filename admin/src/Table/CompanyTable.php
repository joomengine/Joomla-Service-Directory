 ...
 * @since  1.6
 */
class CompanyTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_servicedirectory.company';
		...
		parent::__construct('#__servicedirectory_company', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Company table.
	 *
	...
	public function store($updateNulls = true)
	{
		...
		{
			// New company. A company created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_SERVICEDIRECTORY_COMPANY_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_SERVICEDIRECTORY_COMPANY_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_servicedirectory.company.'.$this->id);
			$this->setRules($rules);
		}
