 ...
 * @since  1.6
 */
class CityTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_servicedirectory.city';
		...
		parent::__construct('#__servicedirectory_city', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the City table.
	 *
	...
	public function store($updateNulls = true)
	{
		...
		{
			// New city. A city created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_SERVICEDIRECTORY_CITY_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_SERVICEDIRECTORY_CITY_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_servicedirectory.city.'.$this->id);
			$this->setRules($rules);
		}
