...
/**
 * Timezones Table class
 *
 * @since  1.6
 */
class TimezoneTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_servicedirectory.timezone';
		...
		parent::__construct('#__servicedirectory_timezone', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Timezone table.
	 *
	...
	public function store($updateNulls = true)
	{
		...
		{
			// New timezone. A timezone created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_SERVICEDIRECTORY_TIMEZONE_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_SERVICEDIRECTORY_TIMEZONE_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_servicedirectory.timezone.'.$this->id);
			$this->setRules($rules);
		}
