...
/**
 * Platforms Table class
 *
 * @since  1.6
 */
class PlatformTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_servicedirectory.platform';
		...
		parent::__construct('#__servicedirectory_platform', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Platform table.
	 *
	...
	public function store($updateNulls = true)
	{
		...
		{
			// New platform. A platform created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_SERVICEDIRECTORY_PLATFORM_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_SERVICEDIRECTORY_PLATFORM_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_servicedirectory.platform.'.$this->id);
			$this->setRules($rules);
		}
