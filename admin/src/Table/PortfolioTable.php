 ...
 * @since  1.6
 */
class PortfolioTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_servicedirectory.portfolio';
		...
		parent::__construct('#__servicedirectory_portfolio', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Portfolio table.
	 *
	...
	public function store($updateNulls = true)
	{
		...
		{
			// New portfolio. A portfolio created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_SERVICEDIRECTORY_PORTFOLIO_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_SERVICEDIRECTORY_PORTFOLIO_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_servicedirectory.portfolio.'.$this->id);
			$this->setRules($rules);
		}
