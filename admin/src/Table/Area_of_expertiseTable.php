...
/**
 * Areas_of_expertise Table class
 *
 * @since  1.6
 */
class Area_of_expertiseTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_servicedirectory.area_of_expertise';
		...
		parent::__construct('#__servicedirectory_area_of_expertise', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Area_of_expertise table.
	 *
	...
	public function store($updateNulls = true)
	{
		...
		{
			// New area_of_expertise. A area_of_expertise created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_SERVICEDIRECTORY_AREA_OF_EXPERTISE_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_SERVICEDIRECTORY_AREA_OF_EXPERTISE_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_servicedirectory.area_of_expertise.'.$this->id);
			$this->setRules($rules);
		}
