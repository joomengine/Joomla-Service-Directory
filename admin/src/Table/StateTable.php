 ...
 * @since  1.6
 */
class StateTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_servicedirectory.state';
		...
		parent::__construct('#__servicedirectory_state', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the State table.
	 *
	...
	public function store($updateNulls = true)
	{
		...
		{
			// New state. A state created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_SERVICEDIRECTORY_STATE_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_SERVICEDIRECTORY_STATE_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_servicedirectory.state.'.$this->id);
			$this->setRules($rules);
		}
