...
/**
 * Review_company_updates Table class
 *
 * @since  1.6
 */
class Review_company_updateTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_servicedirectory.review_company_update';
		...
		parent::__construct('#__servicedirectory_review_company_update', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Review_company_update table.
	 *
	...
	public function store($updateNulls = true)
	{
		...
		{
			// New review_company_update. A review_company_update created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_SERVICEDIRECTORY_REVIEW_COMPANY_UPDATE_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_SERVICEDIRECTORY_REVIEW_COMPANY_UPDATE_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_servicedirectory.review_company_update.'.$this->id);
			$this->setRules($rules);
		}
