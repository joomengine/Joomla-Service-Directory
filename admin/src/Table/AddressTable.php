 ...
 * @since  1.6
 */
class AddressTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_servicedirectory.address';
		...
		parent::__construct('#__servicedirectory_address', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Address table.
	 *
	...
	public function store($updateNulls = true)
	{
		...
		{
			// New address. A address created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_SERVICEDIRECTORY_ADDRESS_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_SERVICEDIRECTORY_ADDRESS_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_servicedirectory.address.'.$this->id);
			$this->setRules($rules);
		}
