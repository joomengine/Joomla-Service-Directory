...
/**
 * File_types Table class
 *
 * @since  1.6
 */
class File_typeTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_servicedirectory.file_type';
		...
		parent::__construct('#__servicedirectory_file_type', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the File_type table.
	 *
	...
	public function store($updateNulls = true)
	{
		...
		{
			// New file_type. A file_type created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_SERVICEDIRECTORY_FILE_TYPE_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_SERVICEDIRECTORY_FILE_TYPE_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_servicedirectory.file_type.'.$this->id);
			$this->setRules($rules);
		}
