 ...
 * @since  1.6
 */
class FileTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_servicedirectory.file';
		...
		parent::__construct('#__servicedirectory_file', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the File table.
	 *
	...
	public function store($updateNulls = true)
	{
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_SERVICEDIRECTORY_FILE_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_SERVICEDIRECTORY_FILE_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_servicedirectory.file.'.$this->id);
			$this->setRules($rules);
		}
