...
class TagTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_servicedirectory.tag';
		...
		parent::__construct('#__servicedirectory_tag', 'id', $db, $dispatcher);
	}
	...
	public function store($updateNulls = true)
	{
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_SERVICEDIRECTORY_TAG_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_SERVICEDIRECTORY_TAG_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_servicedirectory.tag.'.$this->id);
			$this->setRules($rules);
		}
