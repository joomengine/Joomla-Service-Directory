...
/**
 * Servicedirectory List Model for Directory
 *
 * @since  1.6
 */
class DirectoryModel extends ListModel
{
	...
	protected array $styles = [
		...
		'components/com_servicedirectory/assets/css/directory.css'
 	];
	...
	protected function getListQuery()
	{
		...
		// Get from #__servicedirectory_category as a
		$query->select('a.*');
	...
	public function getItems()
	{
		...
		if (!$user->authorise('site.directory.access', 'com_servicedirectory'))
		{
			$app = Factory::getApplication();
			$app->enqueueMessage(Text::_('COM_SERVICEDIRECTORY_NOT_AUTHORISED_TO_VIEW_DIRECTORY'), 'error');
...
/***[JCBGUI.dynamic_get.php_after_getitems.145.$$$$]***/
		...
		if (!empty($items))
		{
			foreach ($items as $nr => &$_item)
			{
				$_item->link = Route::_(RouteHelper::getCategoryRoute($_item->slug));
				// convert category description from markdown to HTML
			...
			foreach ($items as $nr => &$item)
			{
				...
				$this->getDispatcher()->dispatch('onContentPrepare',
					new ContentPrepareEvent(
						'onContentPrepare',
						[
							'context' => 'com_servicedirectory.directory.description',
