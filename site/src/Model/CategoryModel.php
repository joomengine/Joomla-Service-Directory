...
/**
 * Servicedirectory List Model for Category
 *
 * @since  1.6
 */
class CategoryModel extends ListModel
{
	...
	protected array $styles = [
		...
		'components/com_servicedirectory/assets/css/category.css'
 	];
	...
	protected function getListQuery()
	{
		...
		$query->select($db->quoteName(
			array('cc.description','cc.name'),
			array('category_description','category_name')));
	...
	public function getItems()
	{
		...
		if (!$user->authorise('site.category.access', 'com_servicedirectory'))
		{
			$app = Factory::getApplication();
			$app->enqueueMessage(Text::_('COM_SERVICEDIRECTORY_NOT_AUTHORISED_TO_VIEW_CATEGORY'), 'error');
			...
			foreach ($items as $nr => &$item)
			{
				...
				$this->getDispatcher()->dispatch('onContentPrepare',
					new ContentPrepareEvent(
						'onContentPrepare',
						[
							'context' => 'com_servicedirectory.category.category_description',
				...
				$this->getDispatcher()->dispatch('onContentPrepare',
					new ContentPrepareEvent(
						'onContentPrepare',
						[
							'context' => 'com_servicedirectory.category.description',
				...
				$item->guidCompanyCompany_languageL = $this->getGuidCompanyCompany_languageCabf_L($item->guid);
				// set guidCompanyCompany_tagT to the $item object.
				$item->guidCompanyCompany_tagT = $this->getGuidCompanyCompany_tagCabf_T($item->guid);
				// set guidCompanyCompany_area_of_expertiseEE to the $item object.
				$item->guidCompanyCompany_area_of_expertiseEE = $this->getGuidCompanyCompany_area_of_expertiseCabf_EE($item->guid);
			}
		}
	 ...
	 *
	 */
	public function getGuidCompanyCompany_languageCabf_L($guid)
	{
		// Get a db connection.
		$db = $this->getDatabase();
	 ...
	 *
	 */
	public function getGuidCompanyCompany_tagCabf_T($guid)
	{
		// Get a db connection.
		$db = $this->getDatabase();
	 ...
	 *
	 */
	public function getGuidCompanyCompany_area_of_expertiseCabf_EE($guid)
	{
		// Get a db connection.
		$db = $this->getDatabase();
