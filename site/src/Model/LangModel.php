...
/**
 * Servicedirectory List Model for Lang
 *
 * @since  1.6
 */
class LangModel extends ListModel
{
	/**
	...
	protected array $styles = [
		...
		'components/com_servicedirectory/assets/css/lang.css'
 	];
	...
	public function getItems()
	{
		...
		if (!$user->authorise('site.lang.access', 'com_servicedirectory'))
		{
			$app = Factory::getApplication();
			$app->enqueueMessage(Text::_('COM_SERVICEDIRECTORY_NOT_AUTHORISED_TO_VIEW_LANG'), 'error');
			...
			foreach ($items as $nr => &$item)
			{
				...
				$this->getDispatcher()->dispatch('onContentPrepare',
					new ContentPrepareEvent(
						'onContentPrepare',
						[
							'context' => 'com_servicedirectory.lang.category_description',
				...
				$this->getDispatcher()->dispatch('onContentPrepare',
					new ContentPrepareEvent(
						'onContentPrepare',
						[
							'context' => 'com_servicedirectory.lang.description',
				...
				$item->guidCompanyCompany_languageL = $this->getGuidCompanyCompany_languageFfad_L($item->guid);
				// set guidCompanyCompany_tagT to the $item object.
				$item->guidCompanyCompany_tagT = $this->getGuidCompanyCompany_tagFfad_T($item->guid);
				// set guidCompanyCompany_area_of_expertiseEE to the $item object.
				$item->guidCompanyCompany_area_of_expertiseEE = $this->getGuidCompanyCompany_area_of_expertiseFfad_EE($item->guid);
			}
		}
	 ...
	 *
	 */
	public function getGuidCompanyCompany_languageFfad_L($guid)
	{
		// Get a db connection.
		$db = $this->getDatabase();
	 ...
	 *
	 */
	public function getGuidCompanyCompany_tagFfad_T($guid)
	{
		// Get a db connection.
		$db = $this->getDatabase();
	 ...
	 *
	 */
	public function getGuidCompanyCompany_area_of_expertiseFfad_EE($guid)
	{
		// Get a db connection.
		$db = $this->getDatabase();
	...
	public function getLang()
	{
		...
		// Get from #__servicedirectory_language as a
		$query->select($db->quoteName(
			array('a.name'),
			array('name')));
		$query->from($db->quoteName('#__servicedirectory_language', 'a'));
		// Get where a.langtag is $this->entity
		$query->where('a.langtag = ' . $db->quote($this->entity));
