...
/**
 * Servicedirectory List Model for Companies
 *
 * @since  1.6
 */
class CompaniesModel extends ListModel
{
	...
	protected array $styles = [
		...
		'components/com_servicedirectory/assets/css/companies.css'
 	];
...
/***[INSERTED$$$$]***//**437**/
	...
	public function getItems()
	{
		...
		if (!$user->authorise('site.companies.access', 'com_servicedirectory'))
		{
			$app = Factory::getApplication();
			$app->enqueueMessage(Text::_('COM_SERVICEDIRECTORY_NOT_AUTHORISED_TO_VIEW_COMPANIES'), 'error');
			...
			foreach ($items as $nr => &$item)
			{
				...
				$this->getDispatcher()->dispatch('onContentPrepare',
					new ContentPrepareEvent(
						'onContentPrepare',
						[
							'context' => 'com_servicedirectory.companies.category_description',
				...
				$this->getDispatcher()->dispatch('onContentPrepare',
					new ContentPrepareEvent(
						'onContentPrepare',
						[
							'context' => 'com_servicedirectory.companies.description',
				...
				$item->guidCompanyCompany_languageL = $this->getGuidCompanyCompany_languageFcdd_L($item->guid);
				// set guidCompanyCompany_tagT to the $item object.
				$item->guidCompanyCompany_tagT = $this->getGuidCompanyCompany_tagFcdd_T($item->guid);
				// set guidCompanyCompany_area_of_expertiseEE to the $item object.
				$item->guidCompanyCompany_area_of_expertiseEE = $this->getGuidCompanyCompany_area_of_expertiseFcdd_EE($item->guid);
			}
		}
...
/***[INSERTED$$$$]***//**430**/
	 ...
	 *
	 */
	public function getGuidCompanyCompany_languageFcdd_L($guid)
	{
		// Get a db connection.
		$db = $this->getDatabase();
	 ...
	 *
	 */
	public function getGuidCompanyCompany_tagFcdd_T($guid)
	{
		// Get a db connection.
		$db = $this->getDatabase();
	 ...
	 *
	 */
	public function getGuidCompanyCompany_area_of_expertiseFcdd_EE($guid)
	{
		// Get a db connection.
		$db = $this->getDatabase();
