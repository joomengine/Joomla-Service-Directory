...
/**
 * Servicedirectory List Model for Tag
 *
 * @since  1.6
 */
class TagModel extends ListModel
{
	...
	protected array $styles = [
		...
		'components/com_servicedirectory/assets/css/tag.css'
 	];
	...
	public function getItems()
	{
		...
		if (!$user->authorise('site.tag.access', 'com_servicedirectory'))
		{
			$app = Factory::getApplication();
			$app->enqueueMessage(Text::_('COM_SERVICEDIRECTORY_NOT_AUTHORISED_TO_VIEW_TAG'), 'error');
			...
			foreach ($items as $nr => &$item)
			{
				...
				$this->getDispatcher()->dispatch('onContentPrepare',
					new ContentPrepareEvent(
						'onContentPrepare',
						[
							'context' => 'com_servicedirectory.tag.category_description',
				...
				$this->getDispatcher()->dispatch('onContentPrepare',
					new ContentPrepareEvent(
						'onContentPrepare',
						[
							'context' => 'com_servicedirectory.tag.description',
				...
				$item->guidCompanyCompany_languageL = $this->getGuidCompanyCompany_languageAcfa_L($item->guid);
				// set guidCompanyCompany_tagT to the $item object.
				$item->guidCompanyCompany_tagT = $this->getGuidCompanyCompany_tagAcfa_T($item->guid);
				// set guidCompanyCompany_area_of_expertiseEE to the $item object.
				$item->guidCompanyCompany_area_of_expertiseEE = $this->getGuidCompanyCompany_area_of_expertiseAcfa_EE($item->guid);
			}
		}
	 ...
	 *
	 */
	public function getGuidCompanyCompany_languageAcfa_L($guid)
	{
		// Get a db connection.
		$db = $this->getDatabase();
	 ...
	 *
	 */
	public function getGuidCompanyCompany_tagAcfa_T($guid)
	{
		// Get a db connection.
		$db = $this->getDatabase();
	 ...
	 *
	 */
	public function getGuidCompanyCompany_area_of_expertiseAcfa_EE($guid)
	{
		// Get a db connection.
		$db = $this->getDatabase();
