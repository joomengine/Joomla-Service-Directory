...
abstract class UserHelper
{
		...
		array $params = ['useractivation' => 0, 'sendpassword' => 1], int $mode = 1): int
	{
		...
		{
			throw new \InvalidArgumentException(Text::_('COM_SERVICEDIRECTORY_CAN_NOT_SAVE_USER_WITHOUT_EMAIL_VALUE'));
		}
		...
		array $params = ['useractivation' => 0, 'sendpassword' => 1], int $mode = 1): int
	{
		...
		if ($mode === 2 && method_exists(ServicedirectoryHelper::class, 'registerUser'))
		{
			$params['autologin'] = $autologin;
			$userId = ServicedirectoryHelper::registerUser($credentials, $params);
			...
			throw new NoUserIdFoundException(Text::_('COM_SERVICEDIRECTORY_USER_CREATION_FAILED'));
		}
		...
		throw new NoUserIdFoundException(
			Text::sprintf('COM_SERVICEDIRECTORY_USER_S_S_CREATION_FAILEDS',
	...
	public static function update(array $userDetails): int
	{
		...
		throw new \RuntimeException(
			Text::sprintf('COM_SERVICEDIRECTORY_UPDATE_OF_USER_S_S_FAILEDS',
